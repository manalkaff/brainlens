...
import type { Topic, GeneratedContent, VectorDocument } from 'wasp/entities';
 ...
 */
export async function migrateToIterativeResearch(args: any, context: any) {
  if (!context.user) {
  ...
  try {
    ...
    for (const topic of topicsToMigrate) {
      try {
        ...
        const hasRecentResearch = topic.generatedContent.some((content: any) => {
 ...
 */
export async function cleanupOldSystem(args: any, context: any) {
  if (!context.user) {
    throw new HttpError(401, 'Authentication required');
  }
 ...
 */
export async function validateMigration(args: any, context: any) {
  if (!context.user) {
    throw new HttpError(401, 'Authentication required');
  }
  ...
  try {
    ...
    for (const topic of topics) {
      ...
      const hasNewResearch = topic.generatedContent.some((content: any) => {
        const metadata = content.metadata as any;
