...
export function useSubtopicContent(
  ...
  options: UseSubtopicContentOptions = {}
): UseSubtopicContentReturn {
  const {
  ...
  const [content, setContent] = useState<SubtopicContentData | null>(null);
  ...
  const fetchSubtopicContent = useCallback(async (
    mainTopicId: string,
    subtopicId: string,
    signal?: AbortSignal
  ): Promise<SubtopicContentData> => {
    ...
    try {
      ...
      return data as unknown as SubtopicContentData;
    } catch (error: any) {
  ...
  const pollForContent = useCallback(async (mainTopicId: string, subtopicId: string) => {
