...
// Types for aggregation system
export interface AggregatedResult {
  id: string;
  title: string;
  url: string;
  snippet: string;
  sources: string[]; // Which agents found this result
  engines: string[]; // Which search engines provided this result
...
export interface QualityMetrics {
  ...
  sourceReliability: number; // 0-1 score based on source reputation
  recency: number; // 0-1 score based on how recent the content is
  relevance: number; // 0-1 score based on topic relevance
  uniqueness: number; // 0-1 score based on how unique the content is
...
export interface AggregationConfig {
  ...
  minRelevanceScore: number; // Minimum relevance score to include
  minConfidenceScore: number; // Minimum confidence score to include
  boostFactors: {
    multipleAgents: number; // Boost for results found by multiple agents
    highQualitySources: number; // Boost for results from high-quality sources
    recentContent: number; // Boost for recent content
    uniqueContent: number; // Boost for unique content
  };
  penaltyFactors: {
    duplicateContent: number; // Penalty for duplicate content
    lowQualitySources: number; // Penalty for low-quality sources
...
export class ContentAggregator {
   ...
   */
  private collectAllResults(agentResults: ResearchResult[]): SearchResultWithAgent[] {
    const allResults: SearchResultWithAgent[] = [];
   ...
   */
  private async groupDuplicates(results: SearchResultWithAgent[]): Promise<SearchResultGroup[]> {
   ...
   */
  private calculateSimilarity(result1: SearchResultWithAgent, result2: SearchResultWithAgent): number {
   ...
   */
  private async createAggregatedResults(
    ...
    for (const group of groups) {
      ...
      const sourceAttribution: SourceAttribution[] = allResults.map(result => ({
   ...
   */
  private calculateAggregatedRelevance(results: SearchResultWithAgent[]): number {
   ...
   */
  private calculateConfidenceScore(results: SearchResultWithAgent[], duplicateCount: number): number {
    if (results.length === 0) return 0;
  ...
  private async calculateQualityMetrics(
    primary: SearchResultWithAgent,
    allResults: SearchResultWithAgent[],
    context?: any
   ...
   */
  private assessContentQuality(result: SearchResultWithAgent): number {
    ...
    if (content.includes('definition') || content.includes('explanation')) score += 0.1;
    if (content.includes('example') || content.includes('tutorial')) score += 0.1;
   ...
   */
  private assessSourceReliability(result: SearchResultWithAgent): number {
    let score = 0.5; // Base score
   ...
   */
  private assessRecency(result: SearchResultWithAgent): number {
    ...
    try {
      ...
      if (ageInDays <= 365) return 0.7;     // This year
      if (ageInDays <= 365 * 2) return 0.5; // Last 2 years
      if (ageInDays <= 365 * 5) return 0.3; // Last 5 years
   ...
   */
  private assessRelevance(result: SearchResultWithAgent, context?: any): number {
   ...
   */
  private assessUniqueness(result: SearchResultWithAgent, allResults: SearchResultWithAgent[]): number {
    // If this is the only result, it's unique
    if (allResults.length === 1) return 1.0;
   ...
   */
  private applyFilters(results: AggregatedResult[]): AggregatedResult[] {
    return results
   ...
   */
  private selectBestTitle(results: SearchResultWithAgent[]): string {
   ...
   */
  private selectBestSnippet(results: SearchResultWithAgent[]): string {
    // Prefer longer, more informative snippets
    return results
      .sort((a, b) => (b.snippet?.length || 0) - (a.snippet?.length || 0))
      [0]?.snippet || 'No description available';
  }

  /**
   * Select the most recent publication date
   */
  private selectMostRecentDate(results: SearchResultWithAgent[]): string | undefined {
    const dates = results
   ...
   */
  private generateResultId(result: SearchResultWithAgent): string {
  ...
  private generateSummary(
    ...
    const totalOriginalResults = agentResults.reduce(
      (sum, agent) => sum + (agent.results?.length || 0), 0
    );
   ...
   */
  private generateSourceAttribution(agentResults: ResearchResult[]): Record<string, number> {
...
// Supporting types
interface SearchResultWithAgent extends SearchResult {
  agentName: string;
  agentTimestamp: Date;
}
