...
interface OfflineDatabase {
  ...
  settings: { key: string; value: any; updatedAt: Date };
}

export class OfflineStorageManager {
  private db: IDBDatabase | null = null;
  private readonly dbName = 'BrainLensOffline';
   ...
   * Store topic data
   */
  async storeTopic(topic: Topic): Promise<boolean> {
  ...
  async storeContent(
    topicId: string, 
    content: string, 
    contentType: 'markdown' | 'html' | 'json' = 'markdown',
   ...
   */
  async getContent(topicId: string, contentType?: 'markdown' | 'html' | 'json'): Promise<string | null> {
    if (!this.db) return null;
   ...
   * Store user progress
   */
  async storeProgress(progress: UserTopicProgress): Promise<boolean> {
    if (!this.db) return false;
   ...
   */
  async getProgress(topicId: string, userId: string): Promise<UserTopicProgress | null> {
    if (!this.db) return null;
   ...
   * Store chat thread
   */
  async storeThread(thread: ChatThread): Promise<boolean> {
    if (!this.db) return false;
   ...
   */
  async markActionSynced(actionId: string): Promise<boolean> {
    if (!this.db) return false;
