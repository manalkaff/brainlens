...
export function ProgressIndicator({ className, showDetails = false }: ProgressIndicatorProps) {
  ...
  if (!showDetails) {
    ...
    return (
      <div className={`flex items-center space-x-3 ${className}`}>
        ...
        {hierarchyProgress.totalTimeSpent > 0 && (
          <span className="text-sm text-muted-foreground">
            {formatTimeSpent(hierarchyProgress.totalTimeSpent)} spent
          </span>
        )}
        {hierarchyProgress.totalBookmarks > 0 && (
          <span className="text-sm text-muted-foreground">
  ...
  return (
    <Card className={className}>
      ...
      <CardContent className="space-y-4">
        ...
        {/* Depth Progress */}
        {hierarchyProgress.deepestCompletedLevel >= 0 && (
