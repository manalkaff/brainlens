...
interface SubtopicCardsIntegrationExampleProps {
 ...
 */
export function SubtopicCardsIntegrationExample({
  topics,
  initialTopicId
}: SubtopicCardsIntegrationExampleProps) {
  ...
  const handleSubtopicClick = async (subtopic: TopicTreeItem) => {
  ...
  if (isNavigating) {
    return (
      <NavigationLoadingState
        type="navigation"
        message="Navigating to subtopic..."
        progress={navigationProgress}
        topicTitle={selectedSubtopic?.title}
      />
    );
  }
  ...
  return (
    <div className="relative">
      ...
      <ContentDisplay
        ...
        navigationPath={navigationBreadcrumbs}
        ...
        <div className="fixed bottom-4 right-4 p-4 bg-card border rounded-lg shadow-lg max-w-sm">
          ...
          <div className="text-xs space-y-1">
            ...
            <div className="space-y-1">
              {Object.entries(analytics.getAnalyticsSummary().eventTypes).map(([event, count]) => (
                <p key={event} className="text-muted-foreground">
                  {event}: {count}
                </p>
              ))}
            </div>
...
export function SubtopicCardsDemo() {
  ...
  return (
    <div className="p-6">
      ...
      <SubtopicCardsIntegrationExample
        topics={mockTopics}
