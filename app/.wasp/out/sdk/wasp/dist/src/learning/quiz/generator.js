...
export async function generateQuizWithAI(topic, userProgress, vectorDocuments, generatedContent, options) {
    ...
    try {
        ...
        const questions = parsedResponse.map((q, index) => ({
...
function createQuizGenerationPrompt(topic, content, difficulty, questionCount, questionTypes) {
...
async function mockAIQuizGeneration(prompt, topic, difficulty, questionCount, questionTypes) {
...
function generateMockQuestion(topic, type, difficulty, questionNumber) {
...
function generateTemplateBasedQuiz(topic, difficulty, questionCount, questionTypes) {
    const questions = [];
...
export function adjustDifficultyBasedOnPerformance(currentDifficulty, recentScores) {
...
export function getQuestionTypeDistribution(difficulty) {
    switch (difficulty) {
        case 'beginner':
            return [
                QuestionType.MULTIPLE_CHOICE,
