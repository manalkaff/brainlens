...
import { aiContentGenerator, } from "./contentGenerator";
// Stream connection manager
class StreamConnectionManager {
    connections = new Map();
    streamStatus = new Map();
    createStream(streamId, res) {
        // Set SSE headers
        res.writeHead(200, {
...
export class StreamingContentService {
    connectionManager = new StreamConnectionManager();
     ...
     */
    async streamLearningContent(streamId, topic, researchResults, options) {
        try {
            ...
            for await (const chunk of result.textStream) {
                ...
                this.connectionManager.sendToStream(streamId, "content", {
                    chunk,
                    accumulatedContent: accumulatedContent.length > 1000
     ...
     */
    async streamResearchProgress(streamId, topicId, agentNames) {
        try {
            if (!this.connectionManager.isStreamActive(streamId)) {
                throw new Error("Stream connection not found");
            }
     ...
     */
    async streamAssessmentContent(streamId, topic, userPreferences, options) {
        try {
            if (!this.connectionManager.isStreamActive(streamId)) {
                throw new Error("Stream connection not found");
            }
            ...
            const learningPath = await aiContentGenerator.generateAssessmentContent(topic, userPreferences);
    ...
    buildStreamingPrompt(topic, researchResults, options) {
...
export async function createStreamHandler(req, res, context) {
    try {
...
export async function streamContentHandler(req, res, context) {
    try {
        ...
        streamingContentService
            .streamLearningContent(streamId, topic, researchResults || [], options || {})
            .catch((error) => {
