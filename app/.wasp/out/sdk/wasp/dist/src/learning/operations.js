...
import { withDatabaseErrorHandling, validateInput, sanitizeInput, withRetry } from './errors/errorHandler';
import { createAuthenticationError, createValidationError, createLearningError, ErrorType, ErrorSeverity, ERROR_CODES } from './errors/errorTypes';
...
async function generateUniqueSlug(title, context) {
...
export const createTopic = async (args, context) => {
    if (!context.user) {
    ...
    const title = validateInput(args.title, (input) => {
    ...
    return withDatabaseErrorHandling(async () => {
        ...
        if (depth === 0) {
            await withRetry(() => consumeCredits(user.id, 'TOPIC_RESEARCH', context, {
                topicTitle: title,
                parentId: parentId || null
            }), 3, 1000, 'CONSUME_CREDITS');
        }
...
export const getTopic = async (args, context) => {
    if (!context.user) {
        throw createAuthenticationError('Authentication required to access topics');
    }
    // Assert user is defined after authentication check
    const user = context.user;
    const slug = validateInput(args.slug, (input) => {
        if (!input || typeof input !== 'string' || input.trim().length === 0) {
        ...
        return trimmed;
    }, 'slug', { userId: user.id, originalSlug: args.slug });
    return withDatabaseErrorHandling(async () => {
        ...
        if (!topic) {
            throw createLearningError(ErrorType.TOPIC_NOT_FOUND, ERROR_CODES.TOPIC_NOT_FOUND, `Topic with slug '${slug}' not found`, {
...
export const getTopicTree = async (_args, context) => {
    if (!context.user) {
    ...
    return withDatabaseErrorHandling(async () => {
        ...
        const buildTopicTree = async (parentId = null) => {
            ...
            const topicsWithChildren = await Promise.all(topics.map(async (topic) => {
...
export const updateTopicProgress = async (args, context) => {
    if (!context.user) {
    ...
    return withDatabaseErrorHandling(async () => {
        ...
        if (bookmarks !== undefined) {
            ...
            updateData.bookmarks = bookmarks.map((bookmark) => sanitizeInput(bookmark, 500));
        }
        let updatedProgress;
        if (existingProgress) {
        ...
        if (completed === true && topic.parent) {
            await withRetry(() => updateParentProgress(user.id, topic.parent.id, context), 2, 1000, 'UPDATE_PARENT_PROGRESS');
        }
        return updatedProgress;
    }, 'UPDATE_TOPIC_PROGRESS', { userId: user.id, topicId });
};
// Helper function to calculate and update parent topic progress
async function updateParentProgress(userId, parentTopicId, context) {
    try {
        ...
        const completedChildren = parentTopic.children.filter((child) => child.userProgress.length > 0 && child.userProgress[0].completed).length;
...
export const getUserProgressStats = async (_args, context) => {
    if (!context.user) {
...
export const getTopicProgressSummary = async (args, context) => {
    if (!context.user) {
    ...
    return withDatabaseErrorHandling(async () => {
        ...
        const completedChildren = directChildren.filter(child => child.userProgress.length > 0 && child.userProgress[0].completed).length;
        const inProgressChildren = directChildren.filter(child => child.userProgress.length > 0 && !child.userProgress[0].completed && child.userProgress[0].timeSpent > 0).length;
        ...
        const completedDescendants = descendants.filter(d => d.userProgress.length > 0 && d.userProgress[0].completed);
...
export const getSubtopicContentQuery = async ({ mainTopicId, subtopicId: rawSubtopicId, options }, context) => {
    if (!context.user) {
    ...
    return withDatabaseErrorHandling(async () => {
        ...
        if (!subtopic) {
            throw createLearningError(ErrorType.VALIDATION_ERROR, ERROR_CODES.TOPIC_NOT_FOUND, 'The requested subtopic does not exist or does not belong to the specified main topic', {
        ...
        throw createLearningError(ErrorType.VALIDATION_ERROR, ERROR_CODES.TOPIC_NOT_FOUND, `No content found for subtopic "${subtopic.title}". The subtopic may not have been generated yet.`, {
            context: {
                subtopicTitle: subtopic.title,
                parentTopicId: mainTopicId
            },
