...
export function TabNavigation({ className }) {
    ...
    return (<TabsList className={`grid w-full grid-cols-5 lg:w-auto lg:inline-flex ${className}`}>
      {tabConfigs.filter(tab => tab.id !== 'learn').map((tab) => {
            ...
            return (<TabsTrigger key={tab.id} value={tab.id} className="flex items-center space-x-2 relative group" title={tab.description}>
            {/* Tab icon */}
            <div className={`transition-all duration-200 ${isActive ? 'text-foreground' : wasVisited ? 'text-foreground opacity-80' : 'text-muted-foreground'}`}>
              {tab.icon}
            </div>
            
            {/* Tab label */}
            <span className={`transition-all duration-200 ${isActive ? 'font-medium' : wasVisited ? 'font-normal' : 'opacity-75'}`}>
              {tab.label}
            </span>
...
//# sourceMappingURL=TabNavigation.jsx.map