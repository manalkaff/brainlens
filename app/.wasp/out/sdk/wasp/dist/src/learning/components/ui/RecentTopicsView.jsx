...
export function RecentTopicsView({ recentTopics, onTopicSelect, onToggleBookmark, bookmarkedTopics, selectedTopicId }) {
    ...
    return (<div className="p-4 space-y-3">
      ...
      {recentTopics.map((topic, index) => (<Card key={`${topic.id}-${index}`} className={`cursor-pointer transition-all hover:shadow-sm ${selectedTopicId === topic.id ? 'ring-2 ring-primary' : ''}`} onClick={() => onTopicSelect(topic)}>
          <CardContent className="p-3">
            <div className="flex items-start justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={(e) => {
                ...
                {bookmarkedTopics.includes(topic.id) ? (<Bookmark className="w-3 h-3 text-yellow-600 fill-current"/>) : (<BookmarkPlus className="w-3 h-3 text-muted-foreground"/>)}
              </Button>
            </div>
          </CardContent>
        </Card>))}
...
//# sourceMappingURL=RecentTopicsView.jsx.map