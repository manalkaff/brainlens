 ...
 */
export async function checkUserCredits(userId, operation, context) {
 ...
 */
export async function consumeCredits(userId, operation, context, metadata) {
 ...
 */
async function getUserUsageStats(userId, context) {
    const now = new Date();
 ...
 */
function checkFreeTierLimits(operation, usage) {
    const now = new Date();
    switch (operation) {
        case 'TOPIC_RESEARCH':
 ...
 */
function checkHobbyTierLimits(operation, usage) {
    const now = new Date();
 ...
 */
export function checkLearningAdminAccess(user) {
    return user.isAdmin;
}
/**
 * Get upgrade recommendation based on user's usage patterns
 */
export async function getUpgradeRecommendation(userId, context) {
    const user = await context.entities.User.findUnique({
        where: { id: userId }
    });
    if (!user) {
    ...
    return {
        shouldUpgrade: false,
        recommendedPlan: user.subscriptionPlan || PaymentPlanId.Hobby,
