 ...
 */
export const createChatThread = async (args, context) => {
    if (!context.user) {
    ...
    return withDatabaseErrorHandling(async () => {
        ...
        await withAIServiceErrorHandling(() => conversationManager.startConversation(chatThread.id, topicId, topic.title, user.id, userPreferences), 'CONVERSATION_MANAGER', { threadId: chatThread.id, topicId });
        return chatThread;
 ...
 */
export const getChatThread = async (args, context) => {
    if (!context.user) {
        throw createAuthenticationError('Authentication required to get chat thread');
    }
    // Assert user is defined after authentication check
    const user = context.user;
    const threadId = validateInput(args.threadId, (input) => {
        if (!input || typeof input !== 'string') {
    ...
    return withDatabaseErrorHandling(async () => {
        ...
        if (chatThread.topic) {
            await withAIServiceErrorHandling(() => conversationManager.startConversation(chatThread.id, chatThread.topicId, chatThread.topic.title, user.id), 'CONVERSATION_MANAGER', { threadId, topicId: chatThread.topicId });
        }
        return {
            ...chatThread,
 ...
 */
export const getChatThreads = async (args, context) => {
    if (!context.user) {
    ...
    return withDatabaseErrorHandling(async () => {
        ...
        const threadsWithCounts = await Promise.all(chatThreads.map(async (thread) => {
...
export const sendMessage = async (args, context) => {
    ...
    const content = validateInput(args.content, (input) => {
    ...
    }, 'content', { userId: user.id, threadId });
    return withDatabaseErrorHandling(async () => {
        ...
        if (!chatThread.topic) {
            throw createLearningError(ErrorType.CHAT_ERROR, ERROR_CODES.CHAT_MESSAGE_FAILED, 'Chat thread topic not found', {
        ...
        // Start conversation session with error handling
        await withAIServiceErrorHandling(() => conversationManager.startConversation(threadId, chatThread.topicId, chatThread.topic.title, user.id, userPreferences), 'CONVERSATION_MANAGER', { threadId, topicId: chatThread.topicId });
        // Consume credits for AI chat message with retry
        await withRetry(() => consumeCredits(user.id, 'AI_CHAT_MESSAGE', context, {
            threadId,
            topicId: chatThread.topicId,
            messageLength: content.length
        }), 3, 1000, 'CONSUME_CHAT_CREDITS');
        // Process the message and get AI response with error handling
        const result = await withAIServiceErrorHandling(() => conversationManager.processMessage(threadId, content, context), 'MESSAGE_PROCESSING', { threadId, messageLength: content.length });
        // Generate smart question suggestions with error handling
        const smartSuggestions = await withDatabaseErrorHandling(() => generateSmartQuestionSuggestions(user.id, chatThread.topicId, content, result.assistantMessage.content, context), 'GENERATE_SUGGESTIONS', { threadId, topicId: chatThread.topicId }).catch(() => []); // Fallback to empty array if suggestions fail
        ...
        await withRetry(() => updateTopicProgressFromChat(user.id, chatThread.topicId, context), 2, 1000, 'UPDATE_CHAT_PROGRESS').catch((error) => {
 ...
 */
export const updateChatThread = async (args, context) => {
    if (!context.user) {
 ...
 */
async function generateSmartQuestionSuggestions(userId, topicId, userQuestion, assistantResponse, context) {
    try {
 ...
 */
export const exportChatThread = async (args, context) => {
    if (!context.user) {
 ...
 */
async function updateTopicProgressFromChat(userId, topicId, context) {
    try {
