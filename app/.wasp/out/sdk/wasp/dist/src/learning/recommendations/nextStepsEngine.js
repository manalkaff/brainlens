...
class NextStepsEngine {
     ...
     */
    async generateRecommendations(context, metrics, insights, maxRecommendations = 5) {
        // Store context for future use
        this.userContexts.set(context.userId, context);
        // Get applicable strategies
        const applicableStrategies = this.strategies.filter(strategy => strategy.conditions(context, metrics));
        ...
        for (const strategy of applicableStrategies) {
            ...
            recommendations.forEach(rec => {
                rec.strategyWeight = strategy.weight;
            });
            allRecommendations.push(...recommendations);
        }
        // Score and rank recommendations
        const scoredRecommendations = this.scoreRecommendations(allRecommendations, context, metrics, insights);
        // Apply diversity and balance
        const balancedRecommendations = this.applyRecommendationBalance(scoredRecommendations, maxRecommendations);
        const primary = balancedRecommendations[0];
        const alternatives = balancedRecommendations.slice(1);
        const reasoning = this.generateRecommendationReasoning(primary, alternatives, context, metrics);
        return {
            primary,
            alternatives,
            reasoning
     ...
     */
    async getAdaptiveRecommendations(context, currentPerformance) {
     ...
     */
    async generatePathRecommendations(context, currentPath, completedConcepts, metrics) {
        try {
            ...
            if (!conceptMap) {
                ...
                const mockTopic = { id: context.topicId, title: 'Learning Topic' };
            ...
            const optimizedResult = conceptNetworkManager.optimizeConceptOrdering(context.topicId, remainingConcepts, userMastery);
            return {
     ...
     */
    generateHelpRecommendations(context, strugglingConcept, metrics) {
        const recommendations = [];
        // Prerequisites check
        recommendations.push({
    ...
    generatePerformanceBasedRecommendations(context, metrics) {
        const recommendations = [];
    ...
    generateTimeBasedRecommendations(context, metrics) {
        const recommendations = [];
    ...
    generateEngagementRecommendations(context, metrics) {
        const recommendations = [];
    ...
    generateMasteryRecommendations(context, metrics) {
        const recommendations = [];
    ...
    generateRecoveryRecommendations(context, metrics) {
        const recommendations = [];
    ...
    scoreRecommendations(recommendations, context, metrics, insights) {
        return recommendations.map(rec => {
            let score = 0;
            // Base score from strategy weight
            score += rec.strategyWeight || 0.5;
            // Priority weighting
            ...
            if (rec.resources && context.preferences.learningStyle) {
                const styleMatch = rec.resources.some(resource => context.preferences.learningStyle.includes(resource.type));
                if (styleMatch)
                    score += 0.15;
            }
            return { ...rec, score };
    ...
    applyRecommendationBalance(recommendations, maxCount) {
        const balanced = [];
    ...
    generateRecommendationReasoning(primary, alternatives, context, metrics) {
        const reasons = [];
    ...
    async getRemedialResources(conceptId) {
        return [
            {
    ...
    getAlternativeResources(conceptId, learningStyles) {
        const resources = [];
    ...
    getInteractiveResources(learningStyles) {
        return [
            {
...
//# sourceMappingURL=nextStepsEngine.js.map