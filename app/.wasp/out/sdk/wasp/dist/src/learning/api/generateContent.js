...
export const generateContentHandler = async (req, res, context) => {
    try {
        ...
        const now = new Date();
        const isContentFresh = (createdAt) => {
        ...
        try {
            ...
            const researchResult = await iterativeResearchEngine.researchAndGenerate(topic.title, researchOptions, userContext);
...
async function generateSubtopics(topic, context) {
    ...
    if (childTopics.length > 0) {
        return childTopics.map((child) => child.title);
    }
