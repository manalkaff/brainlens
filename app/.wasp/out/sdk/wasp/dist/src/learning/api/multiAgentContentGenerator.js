...
(function (DomainType) {
    DomainType["TECHNOLOGY"] = "technology";
    DomainType["BUSINESS"] = "business";
    DomainType["SCIENCE"] = "science";
    DomainType["MATHEMATICS"] = "mathematics";
    DomainType["ARTS_HUMANITIES"] = "arts_humanities";
    DomainType["HEALTH_MEDICINE"] = "health_medicine";
    DomainType["EDUCATION"] = "education";
    DomainType["SOCIAL_SCIENCES"] = "social_sciences";
    DomainType["ENGINEERING"] = "engineering";
...
export class TopicDomainClassifier {
    ...
    async classifyTopic(topic, researchResults) {
        ...
        const researchContext = researchResults
            ...
            .map((r) => `${r.agent}: ${r.summary || ""} ${r.results
...

{
  ...
  "characteristics": {
    ...
    fallbackClassification(topicText) {
...
export class ResearchResultProcessor {
    processResearchResults(researchResults, domain) {
        return researchResults.map((result) => this.enhanceResearchResult(result, domain));
    }
    enhanceResearchResult(result, domain) {
        ...
        const processedSources = result.results.map((source, index) => this.processSource(source, result.agent, index, domain));
        return {
            ...result,
            processedSources,
            agentWeight,
    ...
    calculateAgentWeight(agentName, domain) {
    ...
    calculateDomainRelevance(result, domain) {
    ...
    processSource(source, agentType, index, domain) {
    ...
    classifySourceContent(source, agentType) {
    ...
    extractSourceMetadata(source, agentType) {
        const metadata = {
        ...
        if (source.metadata) {
            ...
            // Academic metadata
            if (meta.citations)
                metadata.citations = meta.citations;
            if (meta.author)
            ...
            if (meta.difficulty)
                metadata.difficulty = meta.difficulty;
            // Community metadata
            if (meta.upvotes)
                metadata.upvotes = meta.upvotes;
            if (meta.comments)
                metadata.comments = meta.comments;
            if (meta.subreddit)
                metadata.subreddit = meta.subreddit;
            if (meta.sentiment)
                metadata.sentiment = meta.sentiment;
            if (meta.practicalValue)
                metadata.practicalValue = meta.practicalValue;
            ...
            if (meta.complexity)
                metadata.complexity = meta.complexity;
        }
        return metadata;
    }
    calculateTrustScore(source, agentType) {
        ...
        if (agentType === "Video Learning Agent") {
            ...
            if (source.metadata?.isEducational)
                score += 0.1;
        ...
        if (source.url.includes("edu") || source.url.includes("gov"))
            score += 0.2;
        if (source.url.includes("wikipedia"))
            score += 0.1;
        return Math.min(score, 1.0);
    }
    calculateRecency(publishedDate) {
        if (!publishedDate)
            return 0.5;
        try {
...
export class BaseContentGenerator {
    model = openai("gpt-5-nano");
    buildSourceContext(sources, maxSources = 5) {
        const topSources = sources
            .sort((a, b) => b.relevanceScore * (b.metadata.trustScore || 0.5) -
            a.relevanceScore * (a.metadata.trustScore || 0.5))
            .slice(0, maxSources);
        return topSources
            .map((source, index) => `[Source ${index + 1}] ${source.agentType} - "${source.title}"\nURL: ${source.url}\nContent: ${source.content}\nTrust Score: ${source.metadata.trustScore?.toFixed(2)}\nRelevance: ${source.relevanceScore.toFixed(2)}\n`)
            .join("\n");
    }
...
export class OverviewContentGenerator extends BaseContentGenerator {
    async generateContent(topic, sources, domain, context) {
...
export class TechnicalContentGenerator extends BaseContentGenerator {
    async generateContent(topic, sources, domain, context) {
        ...
        const technicalSources = sources
            .filter((source) => source.agentType === "Computational Agent" ||
            source.metadata.hasFormulas ||
            source.metadata.hasAlgorithms ||
            source.contentType === "example")
            .sort((a, b) => {
            const aScore = (a.metadata.hasFormulas ? 0.3 : 0) +
                (a.metadata.hasAlgorithms ? 0.2 : 0) +
                a.relevanceScore * 0.5;
            const bScore = (b.metadata.hasFormulas ? 0.3 : 0) +
...
export class PracticalContentGenerator extends BaseContentGenerator {
    async generateContent(topic, sources, domain, context) {
        ...
        const communitySources = sources
            ...
            .sort((a, b) => {
            const aPractical = a.metadata.practicalValue === "high"
                ? 2
                : a.metadata.practicalValue === "medium"
                    ? 1
                    : 0;
            const bPractical = b.metadata.practicalValue === "high"
                ? 2
...
export class MultiAgentContentOrchestrator {
     ...
     */
    async generateContentSections(topic, sources, domain, subtopics, context) {
     ...
     */
    createGeneratorPlan(domain, sources) {
        const plan = new Map();
        ...
        if (domain.characteristics.includesResearch ||
        ...
        const technicalSources = sources.filter((s) => s.agentType === "Computational Agent" ||
        ...
        if (domain.characteristics.needsVisualExplanations &&
        ...
        if (domain.characteristics.benefitsFromCommunity &&
     ...
     */
    async synthesizeContent(contentSections, topic, domain, subtopics) {
        // Build frontmatter
     ...
     */
    buildFrontmatter(topic, domain, contentSections, subtopics) {
...
---`;
     ...
     */
    determineContentOrder(domain, contentSections) {
        ...
        if (domain.characteristics.emphasizesTheory &&
        ...
        if (domain.characteristics.focusesOnPractical &&
     ...
     */
    buildSubtopicSection(subtopics, domain) {
...
${subtopics
            .map((subtopic) => `- **${subtopic}**: Click to explore this subtopic in detail`)
            .join("\n")}
...
${subtopics
    ...
    buildSourceSection(sources) {
        ...
        const sourcesByAgent = sources.reduce((acc, source) => {
            if (!acc[source.agentType])
                acc[source.agentType] = [];
...
*Content generated using multi-agent research synthesis. Source quality and relevance scores are provided to help evaluate information credibility.*`;
     ...
     */
    buildContentMetadata(domain, enhancedResults, contentSections) {
        const agentContributions = enhancedResults.map((result) => ({
     ...
     */
    estimateReadTime(contentSections) {
        const totalWords = Array.from(contentSections.values()).reduce((sum, section) => sum + (section.content?.split(" ").length || 0), 0);
     ...
     */
    generateTags(domain, subtopics) {
        const tags = [domain.primary];
        // Add secondary domains
        tags.push(...domain.secondary);
        // Add characteristic-based tags
        if (domain.characteristics.requiresCodeExamples)
            tags.push("programming");
        if (domain.characteristics.requiresFormulas)
            tags.push("mathematics");
        if (domain.characteristics.includesResearch)
            tags.push("research");
        if (domain.characteristics.hasIndustryApplications)
            tags.push("industry");
        if (domain.characteristics.focusesOnPractical)
            tags.push("practical");
     ...
     */
    determineDifficulty(domain, contentSections) {
        ...
        if (contentSections.has("academic"))
            difficultyScore += 1;
        if (contentSections.has("technical"))
            difficultyScore += 2;
     ...
     */
    async generateFallbackContent(topic, subtopics, researchResults, context) {
...
*Content generation in progress. The multi-agent research system is gathering information from various sources to provide comprehensive coverage of this topic.*`;
        ...
        const allSources = researchResults.flatMap((r) => r.results.map((source, index) => ({
            id: `fallback_${index}`,
            title: source.title,
            url: source.url,
            content: source.snippet,
...
//# sourceMappingURL=multiAgentContentGenerator.js.map