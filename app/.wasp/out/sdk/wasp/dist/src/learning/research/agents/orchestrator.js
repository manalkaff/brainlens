...
export class ResearchOrchestrator {
     ...
     */
    async orchestrateResearch(topic, context, options = {}) {
        ...
        try {
            ...
            const results = await this.executeAgentsInParallel(session, mergedOptions);
     ...
     */
    async executeAgentsInParallel(session, options) {
        ...
        for (const agent of agents) {
            const agentPromise = semaphore.acquire().then(async (release) => {
                try {
                    return await this.executeAgentWithMonitoring(agent.name, session, options);
                }
                finally {
                    release();
                }
     ...
     */
    async executeAgentWithMonitoring(agentName, session, options) {
        ...
        try {
            ...
            const progressInterval = setInterval(() => {
                this.updateAgentProgress(session, agentName, Math.min(90, (Date.now() - (session.agents.find(a => a.name === agentName)?.startTime?.getTime() || 0)) /
            ...
            if (options.gracefulDegradationEnabled && this.gracefulDegradation) {
                const fallbackResult = await this.gracefulDegradation.handleAgentFailure(agentName, session.topic, session.context, errorMessage);
                if (fallbackResult) {
    ...
    initializeAgentStatuses(priority) {
        const agents = ResearchAgentFactory.getAllAgents();
    ...
    updateAgentStatus(session, agentName, status, error, resultCount) {
    ...
    updateAgentProgress(session, agentName, progress) {
        const agentStatus = session.agents.find(a => a.name === agentName);
        if (agentStatus) {
            agentStatus.progress = Math.min(100, Math.max(0, progress));
        }
    }
    getAgentEngines(agentName) {
        const agent = ResearchAgentFactory.getAgent(agentName);
    ...
    createAgentTimeoutPromise(timeout) {
        return new Promise((_, reject) => {
            setTimeout(() => {
