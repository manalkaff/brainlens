...
export class UtilsModule {
     ...
     */
    buildSourceAttributions(researchResults, sections) {
    ...
    calculateCredibilityScore(result) {
        ...
        if (result.url.includes(".edu") || result.url.includes(".gov"))
            score += 0.3;
        else if (result.url.includes("wikipedia.org"))
            score += 0.2;
        else if (result.url.includes("arxiv.org") || result.url.includes("pubmed"))
            score += 0.4;
        // Title-based indicators
        const title = result.title.toLowerCase();
        if (title.includes("research") || title.includes("study"))
            score += 0.1;
        if (title.includes("peer-reviewed"))
            score += 0.2;
     ...
     */
    classifyContentType(result) {
        const url = result.url.toLowerCase();
        const title = result.title.toLowerCase();
        if (url.includes("youtube.com") || url.includes("video"))
            return "video";
     ...
     */
    estimateSubtopicReadTime(complexity) {
        switch (complexity) {
            case "beginner":
                return 5;
            case "intermediate":
