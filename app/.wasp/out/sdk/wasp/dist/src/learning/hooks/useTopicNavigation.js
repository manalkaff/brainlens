...
const parseURLState = (pathname, search) => {
...
const validateURLState = (state, topics) => {
...
const findTopicBySlugInTree = (topics, slug) => {
...
const getTopicByPathInTree = (topics, path) => {
    if (path.length === 0)
        return null;
...
class TopicContentCache {
    cache = new Map();
    maxSize = 50; // Maximum cached topics
    get(topicId) {
...
const parseDeepLink = (topics, slug, subtopicPath) => {
...
const generateShareableURLUtil = (baseURL, mainTopicSlug, targetTopic, topics) => {
    if (!targetTopic || !topics) {
        return `${baseURL}/learn/${mainTopicSlug}`;
    }
    // Build path to target topic
    const buildPathToTopic = (topicList, targetId, currentPath = []) => {
...
export function useTopicNavigation(topics, initialPath) {
    ...
    const [lastOperations, setLastOperations] = useState(new Map());
    ...
    const findTopicBySlug = useCallback((topicList, slug) => {
        for (const topic of topicList) {
            if (topic.slug === slug)
                return topic;
    ...
    const getTopicByPath = useCallback((path) => {
        if (path.length === 0)
            return null;
        let currentTopics = topics;
        let currentTopic = null;
    ...
    const buildTopicPath = useCallback((topic) => {
        const path = [];
    ...
    const updateURL = useCallback((topic, subtopic, options) => {
        if (!slug)
            return;
        const newState = {};
        if (subtopic) {
    ...
    const addToHistory = useCallback((topic, source = 'sidebar') => {
        const historyItem = {
            topic,
            timestamp: new Date(),
    ...
    const selectTopic = useCallback((topic, source = 'sidebar') => {
        try {
            ...
            setLastOperations(prev => new Map(prev.set(topic.id, () => Promise.resolve(selectTopic(topic, source)))));
        }
        catch (error) {
    ...
    const selectSubtopic = useCallback((subtopic, source = 'sidebar') => {
        try {
            ...
            // Store operation for retry
            setLastOperations(prev => new Map(prev.set(subtopic.id, () => Promise.resolve(selectSubtopic(subtopic, source)))));
        }
        catch (error) {
            errorHandler.handleError(error, {
                type: 'selection',
    ...
    const generateContentForTopic = useCallback(async (topic) => {
        ...
        try {
            ...
            // Store operation for retry
            setLastOperations(prev => new Map(prev.set(topic.id, () => generateContentForTopic(topic))));
        }
        catch (error) {
    ...
    const getTopicContent = useCallback((topicId) => {
        return contentCache.get(topicId);
    }, []);
    const setTopicContent = useCallback((topicId, content, sources) => {
    ...
    const isTopicSelected = useCallback((topicId) => {
    ...
    const getNavigationBreadcrumbs = useCallback(() => {
        const currentTopic = state.selectedSubtopic || state.selectedTopic;
        if (!currentTopic)
            return [];
        const breadcrumbs = [];
    ...
    const getRequiredExpandedNodes = useCallback(() => {
        const currentTopic = state.selectedSubtopic || state.selectedTopic;
        if (!currentTopic)
            return [];
        const path = buildTopicPath(currentTopic);
    ...
    // Handle deep linking
    const handleDeepLink = useCallback((subtopicPath) => {
        if (!slug || !topics.length)
            return false;
    ...
    const validateDeepLink = useCallback((subtopicPath) => {
        if (!slug) {
            return {
                isValid: false,
                mainTopic: null,
    ...
    const generateShareableURL = useCallback((topic) => {
        if (!slug)
    ...
    const retryLastOperation = useCallback(async (topicId) => {
...
//# sourceMappingURL=useTopicNavigation.js.map