...
import { validateTopicInput, sanitizeInput, isSubmitShortcut, INPUT_CONSTRAINTS, classifyError, createRetryHandler, getLoadingMessage } from './utils';
export const InputCard = ({ onSubmit, isLoading = false, placeholder = INPUT_CONSTRAINTS.PLACEHOLDER, maxRetries = 3, showCharacterCount = true, showValidationHints = true }) => {
    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            await retryHandler.current(async () => {
    ...
    return (<div className="w-full max-w-2xl mx-auto px-3 sm:px-4 lg:px-0">
      <div className={`relative bg-card/90 backdrop-blur-sm rounded-2xl shadow-xl border transition-all duration-300 ease-out p-4 sm:p-6 lg:p-8 group ${error
            ...
            : showSuccess
                ...
                : 'border-border/50 hover:border-border focus-within:border-primary/50 focus-within:ring-primary/30'} ${loadingState.isLoading ? 'hover:shadow-xl' : 'hover:shadow-2xl'} focus-within:shadow-2xl focus-within:ring-2`}>
        
        {/* Textarea */}
        <label htmlFor="topic-input" className="sr-only">
          Enter a topic you want to learn about
        </label>
        <textarea id="topic-input" ref={textareaRef} value={input} onChange={handleInputChange} onKeyDown={handleKeyDown} placeholder={placeholder} className={`w-full min-h-[100px] sm:min-h-[120px] lg:min-h-[140px] resize-none border-0 bg-transparent text-base sm:text-lg lg:text-xl text-foreground placeholder-muted-foreground focus:outline-none focus:ring-0 leading-relaxed touch-manipulation transition-all duration-200 ${loadingState.isLoading ? 'opacity-75' : ''}`} disabled={loadingState.isLoading} maxLength={INPUT_CONSTRAINTS.MAX_LENGTH} aria-describedby={[
            'input-instructions',
            error ? 'input-error' : null,
            validation.warnings ? 'input-warnings' : null,
            showCharacterCount ? 'character-count' : null
        ].filter(Boolean).join(' ')} aria-invalid={error ? 'true' : 'false'} aria-required="true" role="textbox" aria-multiline="true"/>
        ...
        <div className="flex items-end justify-between mt-3 sm:mt-4 lg:mt-6 gap-3">
          <div className="flex flex-col space-y-2 sm:space-y-1 text-xs sm:text-sm text-muted-foreground flex-1 min-w-0">
            ...
            <div className="flex items-center space-x-2">
              <kbd className="px-2 py-1 text-xs font-semibold text-muted-foreground bg-muted border border-border rounded whitespace-nowrap transition-colors duration-200 group-focus-within:bg-accent group-focus-within:text-accent-foreground" aria-label="Keyboard shortcut: Command plus Enter">
                ⌘ + Enter
              </kbd>
            ...
            {/* Character count */}
            {showCharacterCount && (<div id="character-count" className="flex items-center space-x-2" aria-live="polite">
                <span className={`text-xs transition-colors duration-200 ${getCharacterCountColor()}`} aria-label={`${input.length} of ${INPUT_CONSTRAINTS.MAX_LENGTH} characters used`}>
                  {input.length}/{INPUT_CONSTRAINTS.MAX_LENGTH}
                </span>
                {input.length > INPUT_CONSTRAINTS.MAX_LENGTH * 0.9 && (<AlertTriangle className="w-3 h-3 text-yellow-600" aria-label="Warning: approaching character limit"/>)}
              </div>)}

            {/* Loading message */}
            {loadingState.isLoading && loadingState.message && (<div className="flex items-center space-x-2 text-xs text-primary" aria-live="polite">
          ...
          <button onClick={handleSubmit} disabled={isSubmitDisabled} type="submit" className={`
            ...
            `} aria-label={loadingState.isLoading
            ? `${loadingState.message || 'Submitting'} - Please wait`
            : showSuccess
                ? 'Topic created successfully!'
                : 'Create learning path for this topic'} aria-describedby="topic-input">
            {loadingState.isLoading ? (<div className="w-5 h-5 sm:w-6 sm:h-6 lg:w-7 lg:h-7 border-2 border-current border-t-transparent rounded-full animate-spin"/>) : showSuccess ? (<CheckCircle className="w-5 h-5 sm:w-6 sm:h-6 lg:w-7 lg:h-7"/>) : (<ArrowRight className="w-5 h-5 sm:w-6 sm:h-6 lg:w-7 lg:h-7 transition-transform duration-200 group-hover:translate-x-0.5"/>)}
          </button>
        </div>


        {/* Error Message with Retry */}
        {error && (<div id="input-error" className="mt-4 p-3 sm:p-4 bg-destructive/10 border border-destructive/20 rounded-xl animate-in slide-in-from-top-2 duration-300" role="alert" aria-live="assertive">
            <div className="flex items-start space-x-2">
              <AlertCircle className="w-4 h-4 text-destructive mt-0.5 flex-shrink-0" aria-hidden="true"/>
              <div className="flex-1">
                <p className="text-sm text-destructive font-medium">{error.message}</p>
                {error.details && (<p className="text-xs text-destructive/70 mt-1">{error.details}</p>)}
                {error.retryable && (<button onClick={handleRetry} className="inline-flex items-center space-x-1 mt-2 text-xs text-destructive hover:text-destructive/80 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-destructive focus:ring-offset-2 focus:ring-offset-background rounded px-2 py-1 min-h-[32px]" type="button" aria-label="Retry submitting the topic">
        ...
        {/* Success Message */}
        {showSuccess && (<div className="mt-4 p-3 sm:p-4 bg-green-50 border border-green-200 rounded-xl animate-in slide-in-from-top-2 duration-300" role="status" aria-live="polite">
            <div className="flex items-center space-x-2">
...
//# sourceMappingURL=InputCard.jsx.map