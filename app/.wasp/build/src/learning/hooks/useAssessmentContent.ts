...
export function useAssessmentContent() {
  const [assessmentContent, setAssessmentContent] = useState<AssessmentContentState>({
    content: null,
    startingPoint: null,
    recommendedPath: [],
    estimatedDuration: 0,
  ...
  const [personalizedPath, setPersonalizedPath] = useState<PersonalizedPathState>({
    path: null,
    isLoading: false,
    error: null
  });
   ...
   */
  const generateContent = useCallback(async (topicId: string, assessment: AssessmentResult) => {
   ...
   */
  const generateStartingPointRecommendation = useCallback(async (
    topicId: string, 
    assessment: AssessmentResult
  ) => {
