...
export function useResearchStreaming(options) {
    const { topicId, onStatusUpdate, onProgressUpdate, onContentUpdate, onError, onComplete, autoReconnect = true, reconnectDelay = 3000, } = { ...DEFAULT_OPTIONS, ...options };
    ...
    const connect = useCallback(() => {
        ...
        try {
            const eventSource = new EventSource(`/api/research/stream?topicId=${encodeURIComponent(topicId)}`);
            eventSource.onopen = () => {
                ...
                try {
                    ...
                    switch (update.type) {
                        case 'status':
                            ...
                            break;
                        case 'progress':
                            const progressData = update.data;
                            setState(prev => ({
                                ...prev,
                        ...
                        case 'error':
                            ...
                            break;
                        case 'complete':
                            const completeData = update.data;
                            setState(prev => ({
                                ...prev,
    ...
    const getEstimatedCompletion = useCallback(() => {
        if (!state.currentStatus || !state.progress)
            return null;
        return StreamingUtils.calculateEstimatedCompletion(state.currentStatus.startTime, state.progress.progress);
...
export function useMultipleResearchStreams(topicIds) {
    const [streams, setStreams] = useState(new Map());
    const addStream = useCallback((topicId, options) => {
    ...
    const removeStream = useCallback((topicId) => {
        setStreams(prev => {
    ...
    const getStreamStatus = useCallback((topicId) => {
...
//# sourceMappingURL=useResearchStreaming.js.map