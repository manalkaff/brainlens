...
import { CreditCard, BookOpen, MessageSquare, Brain, TrendingUp, Clock, Crown, Zap, AlertTriangle } from 'lucide-react';
export function UsageDashboard({ compact = false, showRecommendations = true }) {
    const { data: usageStats, isLoading: isLoadingUsage, error: usageError } = useQuery(getUserUsageStats);
    const { data: recommendation, isLoading: isLoadingRecommendation } = useQuery(getUpgradeRecommendation, undefined, {
    ...
    if (compact) {
        ...
        {usageItems.map((item) => {
                ...
                return (<div key={item.label} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="w-16 h-2 bg-gray-200 rounded-full overflow-hidden">
                  <div className={`h-full transition-all duration-300 ${getProgressColor(item.current, item.limit)}`} style={{ width: `${Math.min(percentage, 100)}%` }}/>
                </div>
              </div>
            </div>);
            })}
    ...
    return (<div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                {usageStats.subscriptionInfo.isSubscribed ? (<Crown className="h-5 w-5 text-yellow-500"/>) : (<CreditCard className="h-5 w-5 text-gray-500"/>)}
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {usageItems.map((item) => {
            ...
            return (<Card key={item.label} className={isNearLimit ? 'border-orange-200 bg-orange-50' : ''}>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="space-y-1">
                  <Progress value={Math.min(percentage, 100)} className={`h-2 ${isNearLimit ? '[&>div]:bg-orange-500' : ''}`}/>
      ...
      {showRecommendations && recommendation && recommendation.shouldUpgrade && (<Alert className="border-blue-200 bg-blue-50">
          <TrendingUp className="h-4 w-4 text-blue-600"/>
          <AlertDescription className="text-blue-800">
            <div className="space-y-2">
              <p className="font-medium">Upgrade Recommended</p>
              <p className="text-sm">{recommendation.reason}</p>
              {recommendation.savings && (<p className="text-xs">
      ...
      <div className="flex gap-2">
        ...
        {!usageStats.subscriptionInfo.isSubscribed && (<Button asChild size="sm">
            <Link to="/pricing">
...
//# sourceMappingURL=UsageDashboard.jsx.map