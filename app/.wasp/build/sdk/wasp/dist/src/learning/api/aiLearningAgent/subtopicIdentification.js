...
import { SubtopicsSchema } from "./types";
/**
...
export class SubtopicIdentificationModule {
     ...
     */
    async identifySubtopics(topic, synthesis, nextDepth) {
...
- Do not add subtopics from your own knowledge - only those that emerged from the research data`;
        try {
            ...
            const processedSubtopics = result.object.subtopics.map((subtopic) => ({
                ...subtopic,
     ...
     */
    createFallbackSubtopics(topic, synthesis) {
        const themes = synthesis.contentThemes || [];
        const fallbackSubtopics = themes.slice(0, 5).map((theme, index) => ({
            title: theme,
...
//# sourceMappingURL=subtopicIdentification.js.map