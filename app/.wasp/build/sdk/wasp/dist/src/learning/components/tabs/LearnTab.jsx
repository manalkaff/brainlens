...
import { KnowledgeAssessment } from '../ui/KnowledgeAssessment';
import { StartingPointRecommendation } from '../ui/StartingPointRecommendation';
import { StreamingContent } from '../ui/StreamingContent';
import { ConceptExpansion } from '../ui/ConceptExpansion';
import { updateTopicProgress
...
export function LearnTab() {
    ...
    useEffect(() => {
        if (userProgress?.preferences && Object.keys(userProgress.preferences).length > 0) {
            ...
            const storedPrefs = userProgress.preferences;
    ...
    const handleStartLearning = async (path) => {
        if (!topic || !user || !assessment)
            return;
        setIsSaving(true);
        try {
    ...
    const handleConceptExpand = (concept) => {
    ...
    if (currentPhase === 'assessment') {
        return (<div className="space-y-6">
        <KnowledgeAssessment onComplete={handleAssessmentComplete} isLoading={isSaving}/>
      </div>);
    }
    if (currentPhase === 'recommendation' && assessment) {
        return (<div className="space-y-6">
        <StartingPointRecommendation assessment={assessment} onStartLearning={handleStartLearning} isLoading={isSaving}/>
      </div>);
    }
    ...
    return (<div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {assessment && (<div className="flex flex-wrap gap-3">
            ...
            {learningProgress > 0 && (<div className="space-y-2">
      ...
      {selectedPath && (<div className="lesson-content">
          <StreamingContent topic={{
                id: topic.id,
                title: topic.title,
                summary: topic.summary || undefined
            }} assessment={assessment} selectedPath={selectedPath} onProgressUpdate={handleProgressUpdate} onConceptExpand={handleConceptExpand}/>
        </div>)}

      {/* Concept Expansion Modal */}
      <Dialog open={!!expandedConcept} onOpenChange={() => handleConceptClose()}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          {expandedConcept && (<ConceptExpansion concept={expandedConcept} topicTitle={topic.title} onClose={handleConceptClose} onNavigateToSubtopic={handleNavigateToSubtopic}/>)}
        </DialogContent>
      </Dialog>
    </div>);
}
//# sourceMappingURL=LearnTab.jsx.map