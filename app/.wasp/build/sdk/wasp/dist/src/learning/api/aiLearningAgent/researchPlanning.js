...
import { ResearchPlanSchema } from "./types";
/**
...
export class ResearchPlanningModule {
     ...
     */
    async planResearch(topic, understanding, userContext) {
...
Your goal: Create a comprehensive research plan that balances general accessibility (via 5 general queries) with specialized depth (via recommended engine queries), following the research-driven recommendations above.`;
     ...
     */
    ensureGeneralQueries(plan, topic) {
        ...
        const generalQueries = plan.researchQueries.filter((q) => q.engine === "general");
        const communityQueries = plan.researchQueries.filter((q) => q.engine === "community");
        const videoQueries = plan.researchQueries.filter((q) => q.engine === "video");
        const otherQueries = plan.researchQueries.filter((q) => !['general', 'community', 'video'].includes(q.engine));
        ...
        const actualGeneral = plan.researchQueries.filter((q) => q.engine === "general").length;
        const actualCommunity = plan.researchQueries.filter((q) => q.engine === "community").length;
        const actualVideo = plan.researchQueries.filter((q) => q.engine === "video").length;
        if (actualGeneral < 5) {
     ...
     */
    validateQueryDiversity(queries, topic) {
        ...
        const hasAccessibleTerms = generalQueries.some(q => q.query.includes("basics") ||
        ...
        const hasSpecializedTerms = specializedQueries.length > 0 ||
            queries.some(q => q.query.includes("research") ||
     ...
     */
    generateAdditionalGeneralQueries(topic, count) {
     ...
     */
    generateAdditionalCommunityQueries(topic, count) {
     ...
     */
    generateAdditionalVideoQueries(topic, count) {
     ...
     */
    calculateEngineDistribution(queries) {
        const distribution = {
            general: 0,
            academic: 0,
            video: 0,
     ...
     */
    createFallbackPlan(topic, understanding) {
...
//# sourceMappingURL=researchPlanning.js.map