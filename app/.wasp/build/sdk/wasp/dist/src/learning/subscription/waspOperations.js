...
import { checkUserCredits, consumeCredits, getUpgradeRecommendation as getUpgradeRec, checkLearningAdminAccess, CREDIT_COSTS, FREE_TIER_LIMITS, PREMIUM_TIER_LIMITS } from './operations';
import { SubscriptionStatus } from '../../payment/plans';
export const checkLearningCredits = async (args, context) => {
    if (!context.user) {
...
export const getUserUsageStats = async (_args, context) => {
    if (!context.user) {
        throw new HttpError(401, 'Authentication required');
    }
    // Assert user is defined after authentication check
...
export const getUpgradeRecommendation = async (_args, context) => {
    if (!context.user) {
        throw new HttpError(401, 'Authentication required');
    }
    // Assert user is defined after authentication check
...
export const consumeLearningCredits = async (args, context) => {
    if (!context.user) {
        throw new HttpError(401, 'Authentication required');
    }
    // Assert user is defined after authentication check
...
export const getLearningAnalytics = async (_args, context) => {
    if (!context.user) {
        throw new HttpError(401, 'Authentication required');
    }
...
export const updateUserLearningQuota = async (args, context) => {
    if (!context.user) {
        throw new HttpError(401, 'Authentication required');
    }
    if (!checkLearningAdminAccess(context.user)) {
...
//# sourceMappingURL=waspOperations.js.map