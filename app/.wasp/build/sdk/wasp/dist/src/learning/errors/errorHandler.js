...
import { ErrorType, ErrorSeverity, classifyError, logError, createLearningError, ERROR_CODES } from './errorTypes';
// Server-side error handler
export function handleServerError(error, operation, context) {
...
export async function withDatabaseErrorHandling(operation, operationName, context) {
    try {
        ...
        if (error instanceof Error) {
            ...
            if (error.message.includes('connection')) {
                const dbError = createLearningError(ErrorType.DATABASE_CONNECTION_ERROR, ERROR_CODES.DB_CONNECTION_FAILED, `Database connection failed during ${operationName}`, {
            ...
            if (error.message.includes('timeout')) {
                const timeoutError = createLearningError(ErrorType.DATABASE_TIMEOUT_ERROR, ERROR_CODES.DB_TIMEOUT, `Database timeout during ${operationName}`, {
            ...
            if (error.message.includes('unique constraint') || error.message.includes('duplicate')) {
                const constraintError = createLearningError(ErrorType.DATABASE_CONSTRAINT_ERROR, ERROR_CODES.DB_DUPLICATE_ENTRY, `Duplicate entry during ${operationName}`, {
        ...
        handleServerError(error, operationName, context);
    }
}
// AI service operation wrapper with error handling
export async function withAIServiceErrorHandling(operation, serviceName, context) {
    try {
        return await operation();
    }
    catch (error) {
        if (error instanceof Error) {
            ...
            if (message.includes('rate limit') || message.includes('429')) {
                const rateLimitError = createLearningError(ErrorType.AI_RATE_LIMIT_ERROR, ERROR_CODES.AI_RATE_LIMIT_EXCEEDED, `Rate limit exceeded for ${serviceName}`, {
                    severity: ErrorSeverity.MEDIUM,
                    recoverable: true,
                    retryAfter: 60000,
            ...
            if (message.includes('quota') || message.includes('limit')) {
                const quotaError = createLearningError(ErrorType.AI_QUOTA_EXCEEDED, ERROR_CODES.AI_QUOTA_EXCEEDED, `Quota exceeded for ${serviceName}`, {
                    severity: ErrorSeverity.HIGH,
                    recoverable: false,
            ...
            if (message.includes('content') && message.includes('filter')) {
                const contentFilterError = createLearningError(ErrorType.AI_CONTENT_FILTER_ERROR, ERROR_CODES.AI_CONTENT_FILTERED, `Content filtered by ${serviceName}`, {
                    severity: ErrorSeverity.LOW,
                    recoverable: false,
        ...
        const aiError = createLearningError(ErrorType.AI_API_ERROR, ERROR_CODES.AI_API_UNAVAILABLE, `AI service error: ${serviceName}`, {
            severity: ErrorSeverity.MEDIUM,
            recoverable: true,
            retryAfter: 10000,
...
export async function withVectorStoreErrorHandling(operation, operationName, context) {
    try {
        return await operation();
    }
    catch (error) {
        if (error instanceof Error) {
            ...
            if (message.includes('connection') || message.includes('connect')) {
                const connectionError = createLearningError(ErrorType.VECTOR_STORE_ERROR, ERROR_CODES.VECTOR_CONNECTION_FAILED, `Vector store connection failed during ${operationName}`, {
                    severity: ErrorSeverity.MEDIUM,
                    recoverable: true,
                    retryAfter: 5000,
            ...
            if (message.includes('embedding')) {
                const embeddingError = createLearningError(ErrorType.EMBEDDING_GENERATION_ERROR, ERROR_CODES.VECTOR_EMBEDDING_FAILED, `Embedding generation failed during ${operationName}`, {
                    severity: ErrorSeverity.MEDIUM,
                    recoverable: true,
                    retryAfter: 3000,
        ...
        const vectorError = createLearningError(ErrorType.VECTOR_STORE_ERROR, ERROR_CODES.VECTOR_SEARCH_FAILED, `Vector store operation failed: ${operationName}`, {
            severity: ErrorSeverity.MEDIUM,
            recoverable: true,
            retryAfter: 5000,
...
export async function withResearchPipelineErrorHandling(operation, stage, context) {
    try {
        return await operation();
    }
    catch (error) {
        const researchError = createLearningError(ErrorType.RESEARCH_PIPELINE_ERROR, ERROR_CODES.RESEARCH_INITIALIZATION_FAILED, `Research pipeline error at stage: ${stage}`, {
            severity: ErrorSeverity.MEDIUM,
            recoverable: true,
            retryAfter: 15000,
...
export function validateInput(input, validator, fieldName, context) {
    try {
        return validator(input);
    }
    catch (error) {
        const validationError = createLearningError(ErrorType.VALIDATION_ERROR, ERROR_CODES.VALIDATION_INVALID_FORMAT, `Validation failed for field: ${fieldName}`, {
            severity: ErrorSeverity.LOW,
            recoverable: false,
...
export function sanitizeInput(input, maxLength = 1000, allowedCharacters) {
    try {
        ...
        const sanitizationError = createLearningError(ErrorType.INPUT_SANITIZATION_ERROR, ERROR_CODES.VALIDATION_INVALID_CHARACTERS, 'Input sanitization failed', {
            severity: ErrorSeverity.LOW,
            recoverable: false,
...
export async function withRetry(operation, maxRetries = 3, baseDelay = 1000, operationName = 'operation') {
    let lastError = null;
    ...
    const finalError = createLearningError(ErrorType.INTERNAL_SERVER_ERROR, ERROR_CODES.INTERNAL_ERROR, `Operation failed after ${maxRetries} attempts: ${operationName}`, {
        severity: ErrorSeverity.HIGH,
        recoverable: false,
    ...
    async execute(operation, serviceName) {
        if (this.state === 'OPEN') {
            if (Date.now() - this.lastFailureTime < this.recoveryTimeout) {
                throw createLearningError(ErrorType.SERVICE_UNAVAILABLE, ERROR_CODES.SERVICE_UNAVAILABLE, `Service ${serviceName} is temporarily unavailable`, {
                    severity: ErrorSeverity.MEDIUM,
                    recoverable: true,
...
export async function withGracefulDegradation(primaryOperation, fallbackOperation, operationName) {
    try {
