...
export function useStreamingContent() {
    ...
    const [researchTopicId, setResearchTopicId] = useState(null);
    ...
    const researchStreaming = researchTopicId ? useResearchStreaming({
        ...
        onContentUpdate: (content) => {
            ...
            setSections(prev => prev.map((section, index) => index === currentSectionIndex && content.agent
                ? {
                    ...section,
    ...
    const complete = useCallback(async (prompt) => {
        ...
        try {
            ...
            setSections(prev => prev.map((section, index) => index === currentSectionIndex
    ...
    const createPrompt = (options, section) => {
        const difficultyMap = {
            1: 'complete beginner',
            2: 'novice',
            3: 'intermediate',
        ...
        const difficulty = difficultyMap[options.knowledgeLevel] || 'beginner';
        return `
...
Generate the content now:
    ...
    const regenerateSection = useCallback(async (sectionId, options) => {
    ...
    const getOverallStatus = useCallback(() => {
        ...
        if (isGenerating)
            return 'generating';
        if (sections.length > 0 && sections.every(s => s.isComplete))
            return 'complete';
        if (sections.length > 0)
            return 'partial';
        return 'idle';
...
//# sourceMappingURL=useStreamingContent.js.map