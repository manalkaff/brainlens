...
export function ContextualTooltip({ title, content, position = 'top', trigger, className = '' }) {
    const [isVisible, setIsVisible] = useState(false);
    const defaultTrigger = (<Button variant="ghost" size="sm" className="h-6 w-6 p-0 text-muted-foreground hover:text-foreground" onClick={() => setIsVisible(!isVisible)}>
          ...
          <Card className={`absolute z-50 w-80 max-w-sm shadow-lg ${positionClasses[position]}`}>
            <CardContent className="p-4">
              <div className="flex items-start justify-between mb-2">
                ...
                <Button variant="ghost" size="sm" className="h-6 w-6 p-0 -mt-1 -mr-1" onClick={() => setIsVisible(false)}>
                  <X className="h-4 w-4"/>
                </Button>
              </div>
...
export function QuickHelp({ type, position = 'top', className }) {
    const tooltip = HelpTooltips[type];
    return (<ContextualTooltip title={tooltip.title} content={tooltip.content} position={position} className={className}/>);
}
//# sourceMappingURL=ContextualTooltip.jsx.map