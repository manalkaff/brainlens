...
export class SubtopicExtractor {
     ...
     */
    async extractSubtopics(researchResults, synthesisResult, mainTopic, context) {
        const startTime = Date.now();
        try {
            // Step 1: Extract candidate subtopics from all sources
            const candidateTopics = await this.extractCandidateTopics(researchResults, synthesisResult, mainTopic, context);
            // Step 2: Build hierarchical relationships
            const relationships = await this.buildTopicRelationships(candidateTopics, mainTopic);
            // Step 3: Create hierarchical structure
            const hierarchicalTopics = this.buildHierarchicalStructure(candidateTopics, relationships, mainTopic);
            // Step 4: Enhance with metadata
            const enhancedTopics = await this.enhanceTopicsWithMetadata(hierarchicalTopics, researchResults, context);
     ...
     */
    async extractCandidateTopics(researchResults, synthesisResult, mainTopic, context) {
        const extractionPrompt = this.createExtractionPrompt(researchResults, synthesisResult, mainTopic, context);
     ...
     */
    createExtractionPrompt(researchResults, synthesisResult, mainTopic, context) {
...
- Maintain high confidence scores (>0.7) for all extracted topics`;
     ...
     */
    parseExtractedTopics(topics) {
        const parsed = [];
        let idCounter = 1;
        const processTopicLevel = (topic, level, parentId) => {
            ...
            if (topic.subtopics && Array.isArray(topic.subtopics)) {
                extractedTopic.children = topic.subtopics.map((subtopic) => processTopicLevel(subtopic, level + 1, topicId));
            }
     ...
     */
    async buildTopicRelationships(topics, mainTopic) {
        const relationships = [];
        ...
        for (let i = 0; i < flatTopics.length; i++) {
            for (let j = i + 1; j < flatTopics.length; j++) {
                const relationship = await this.analyzeTopicRelationship(flatTopics[i], flatTopics[j], mainTopic);
     ...
     */
    async analyzeTopicRelationship(topic1, topic2, mainTopic) {
        ...
        if (topic1.metadata.prerequisites.some((prereq) => text2.includes(prereq.toLowerCase()))) {
            return {
        ...
        if (topic2.metadata.prerequisites.some((prereq) => text1.includes(prereq.toLowerCase()))) {
            return {
        ...
        const commonTerms = topic1.metadata.keyTerms.filter((term) => topic2.metadata.keyTerms.some((otherTerm) => term.toLowerCase().includes(otherTerm.toLowerCase()) ||
     ...
     */
    buildHierarchicalStructure(topics, relationships, mainTopic) {
     ...
     */
    async enhanceTopicsWithMetadata(topics, researchResults, context) {
        const enhanceRecursively = (topic) => {
            ...
            if (context?.userLevel) {
                topic.metadata.difficulty = this.adjustDifficultyForUser(topic.metadata.difficulty, context.userLevel);
            }
     ...
     */
    findRelevantAgents(topic, researchResults) {
        const topicTerms = [
     ...
     */
    extractRelatedConcepts(topic, researchResults) {
     ...
     */
    adjustDifficultyForUser(originalDifficulty, userLevel) {
        const difficultyMap = {
            beginner: 1,
            intermediate: 2,
            advanced: 3,
     ...
     */
    filterAndValidateTopics(topics, context) {
        const filterRecursively = (topic) => {
            ...
            if (context?.focusAreas && context.focusAreas.length > 0) {
                ...
                const hasFocus = context.focusAreas.some((area) => topicContent.includes(area.toLowerCase()));
            ...
            if (context?.excludeAreas && context.excludeAreas.length > 0) {
                const topicContent = `${topic.title} ${topic.description}`.toLowerCase();
                const shouldExclude = context.excludeAreas.some((area) => topicContent.includes(area.toLowerCase()));
                if (shouldExclude) {
                    return null;
                }
            }
    ...
    enforceMaxTopics(topics) {
        ...
        const flatSorted = this.flattenHierarchy(topics).sort((a, b) => b.metadata.confidence - a.metadata.confidence);
        const keepIds = new Set(flatSorted.slice(0, this.config.maxSubtopics).map((t) => t.id));
        const filterByIds = (topic) => {
            if (!keepIds.has(topic.id))
                return null;
            if (topic.children) {
                topic.children = topic.children
                    .map(filterByIds)
    ...
    // Helper methods
    flattenHierarchy(topics) {
        const flattened = [];
        const flattenRecursively = (topic) => {
    ...
    calculateAverageConfidence(topics) {
        if (topics.length === 0)
            return 0;
        const totalConfidence = topics.reduce((sum, topic) => sum + topic.metadata.confidence, 0);
        return totalConfidence / topics.length;
    }
    calculateCoverageMetrics(topics, researchResults) {
        const academic = topics.filter((t) => t.metadata.sourceAgents.some((agent) => agent.toLowerCase().includes("academic"))).length / Math.max(1, topics.length);
        const practical = topics.filter((t) => t.metadata.practicalApplications.length > 0).length /
            Math.max(1, topics.length);
        const foundational = topics.filter((t) => t.metadata.difficulty === "beginner").length /
            Math.max(1, topics.length);
        const advanced = topics.filter((t) => t.metadata.difficulty === "advanced").length /
            Math.max(1, topics.length);
...
//# sourceMappingURL=subtopicExtractor.js.map