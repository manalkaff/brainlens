...
export function KnowledgeAssessment({ onComplete, isLoading = false }) {
    ...
    const handleStyleToggle = (styleId) => {
        setSelectedStyles(prev => prev.includes(styleId)
      ...
      <CardContent className="space-y-6">
        ...
        {currentStep === 1 && (<div className="space-y-4">
            ...
            <RadioGroup value={knowledgeLevel} onValueChange={setKnowledgeLevel}>
              {KNOWLEDGE_LEVELS.map((level) => (<div key={level.id} className="flex items-start space-x-3 p-3 border rounded-lg hover:bg-muted/50">
        ...
        {currentStep === 2 && (<div className="space-y-4">
            ...
            <div className="grid gap-3">
              {LEARNING_STYLES.map((style) => (<div key={style.id} className={`flex items-start space-x-3 p-3 border rounded-lg cursor-pointer transition-colors ${selectedStyles.includes(style.id)
                    ? 'border-primary bg-primary/5'
                    : 'hover:bg-muted/50'}`} onClick={() => handleStyleToggle(style.id)}>
                  <Checkbox checked={selectedStyles.includes(style.id)} onChange={() => handleStyleToggle(style.id)} className="mt-1"/>
                  <div className="flex-1">
        ...
        <div className="flex justify-between pt-4 border-t">
          ...
          {currentStep < totalSteps ? (<Button onClick={handleNext} disabled={!canProceed() || isLoading}>
              Next
            </Button>) : (<Button onClick={handleComplete} disabled={!canProceed() || isLoading}>
...
//# sourceMappingURL=KnowledgeAssessment.jsx.map