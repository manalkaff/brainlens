...
export function QuizHistory({ quizzes, onViewQuiz, onRetakeQuiz }) {
    ...
    const getImprovementTrend = (quizzes) => {
        const sortedQuizzes = quizzes
            .filter(q => q.completed && q.score !== null)
            .sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
        if (sortedQuizzes.length < 2)
            return null;
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  {trend && (<Badge variant="outline" className={trend.isImproving ? 'text-green-600 border-green-200' :
                        trend.isStable ? 'text-blue-600 border-blue-200' :
                            'text-red-600 border-red-200'}>
                      <TrendingUp className={`w-3 h-3 mr-1 ${trend.isImproving ? '' :
                        trend.isStable ? 'rotate-90' : 'rotate-180'}`}/>
                      {trend.isImproving ? '+' : trend.isStable ? '±' : ''}
                      {Math.abs(Math.round(trend.improvement))}%
                    </Badge>)}
                  <Button variant="outline" size="sm" onClick={() => onRetakeQuiz(topicId)}>
                    New Quiz
                  </Button>
                </div>
              </div>
            ...
            <CardContent>
              <div className="space-y-3">
                {topicQuizzes
                    ...
                    .map((quiz) => (<div key={quiz.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex-1 space-y-1">
                        <div className="flex items-center space-x-2">
                          <span className="font-medium text-sm">{quiz.title}</span>
                          {quiz.completed ? (<Badge className={`text-xs ${getPerformanceColor(quiz.score || 0)}`}>
                        ...
                        {quiz.completed && quiz.score !== null && (<Progress value={quiz.score} className="w-full h-1"/>)}
                      </div>
                      <Button variant="ghost" size="sm" onClick={() => onViewQuiz(quiz)} className="ml-4">
...
//# sourceMappingURL=QuizHistory.jsx.map