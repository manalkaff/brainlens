...
import { ContentStructureSchema } from "./types";
/**
...
Create content that helps readers truly understand ${topic} and how to apply it in practice.`;
        ...
        try {
            ...
            console.error("Structured generation failed, attempting enhanced fallback:", error);
            ...
            try {
                ...
                const fallbackContent = this.createBasicFallbackContent(topic, synthesis, error);
        ...
        const cleanSections = content.sections.map((section, index) => ({
            ...section,
        ...
        const estimatedReadTime = this.estimateReadTime(cleanSections.reduce((total, section) => total + section.content.length, 0));
     ...
     */
    formatCommunityInsight(insight) {
        let formatted = '';
        switch (insight.type) {
            case 'opinion':
     ...
     */
    inferSectionComplexity(index, totalSections) {
     ...
     */
    validateProgressiveLearningStructure(sections) {
        ...
        const hasFoundationalStart = earlyTitles.some(title => title.includes('basic') ||
        ...
        const lateTitles = sections.slice(Math.floor(sections.length * 2 / 3))
            .map(s => s.title.toLowerCase());
        const hasPracticalEnd = lateTitles.some(title => title.includes('application') ||
    ...
    parseContentFallback(text, topic) {
     ...
     */
    createBasicFallbackContent(topic, synthesis, error) {
        ...
        return {
            ...
            keyTakeaways,
            nextSteps,
            estimatedReadTime: this.estimateReadTime(sections.reduce((total, section) => total + section.content.length, 0))
        };
    }
    createFoundationContent(topic, insights) {
    ...
    createBuildingContent(topic, insights, themes) {
    ...
    createApplicationContent(topic, insights, themes) {
    ...
    createCommunitySection(topic, insights, themes) {
    ...
    createSampleCommunityInsights(topic, section) {
