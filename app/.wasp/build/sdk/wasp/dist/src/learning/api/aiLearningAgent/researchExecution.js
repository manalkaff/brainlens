...
export class ResearchExecutionModule {
     ...
     */
    async executeResearch(researchPlan) {
        ...
        const researchPromises = researchPlan.researchQueries.map(async ({ query, engine, reasoning }, index) => {
            ...
            try {
                ...
                const response = await SearxngUtils.searchWithAgent(engine, query);
        ...
        this.validateResearchExecutionSuccess(generalQueriesSuccessful, specializedQueriesSuccessful, failedQueries, deduplicatedResults.length);
     ...
     */
    async handleGeneralEngineFailure(query, reasoning, errorMessage) {
     ...
     */
    async handleSpecializedEngineFailure(query, engine, reasoning, errorMessage) {
     ...
     */
    validateResearchExecutionSuccess(generalQueriesSuccessful, specializedQueriesSuccessful, failedQueries, totalResults) {
        const criticalFailures = [];
        const warnings = [];
        // Critical validation: ensure minimum general queries succeeded (requirement 5.5)
        if (generalQueriesSuccessful < 3) {
            criticalFailures.push(`Insufficient general engine queries succeeded (${generalQueriesSuccessful} < 3). ` +
        ...
        if (specializedFailureRate > 0 && specializedQueriesSuccessful === 0) {
            warnings.push(`All specialized engine queries failed. Research will rely entirely on general sources.`);
        }
        // Critical validation: ensure we have minimum viable results
        if (totalResults < 5) {
            criticalFailures.push(`Insufficient research results collected (${totalResults} < 5). ` +
...
//# sourceMappingURL=researchExecution.js.map