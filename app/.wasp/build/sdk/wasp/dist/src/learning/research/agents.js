...
import { SearxngUtils, isSearxngError, getSearxngErrorMessage } from './searxng';
...
class BaseResearchAgent {
    ...
    async generateSummary(results, topic) {
    ...
    async identifySubtopics(results, topic) {
...
export class GeneralResearchAgent extends BaseResearchAgent {
    ...
    engines = []; // No specific engine constraints
    ...
    async performSearch(topic, context) {
        try {
...
export class AcademicResearchAgent extends BaseResearchAgent {
    ...
    extractCitations(result) {
        ...
        if (result.citations)
            return parseInt(result.citations);
        if (result.metadata?.citations)
            return parseInt(result.metadata.citations);
...
export class ComputationalAgent extends BaseResearchAgent {
    ...
    assessComplexity(result, userLevel) {
        const content = (result.content || result.snippet || '').toLowerCase();
        // Advanced indicators
...
export class VideoLearningAgent extends BaseResearchAgent {
    ...
    assessDifficulty(result, userLevel) {
        const content = (result.title + ' ' + (result.content || result.snippet || '')).toLowerCase();
        // Advanced indicators
    ...
    matchesUserLevel(videoDifficulty, userLevel) {
        if (!videoDifficulty || !userLevel)
            return true;
        // Exact match
        if (videoDifficulty === userLevel)
            return true;
        ...
        if (userLevel === 'intermediate')
            return true; // Intermediate can handle any level
        if (userLevel === 'advanced' && videoDifficulty !== 'beginner')
            return true;
        if (userLevel === 'beginner' && videoDifficulty === 'beginner')
            return true;
        return false;
    }
    deduplicateResults(results) {
...
export class CommunityDiscussionAgent extends BaseResearchAgent {
    ...
    extractUpvotes(result) {
        // Try to extract upvote count from various fields
        if (result.upvotes)
            return parseInt(result.upvotes);
        if (result.score)
    ...
    extractCommentCount(result) {
        // Try to extract comment count from various fields
        if (result.comments)
            return parseInt(result.comments);
...
export class ResearchAgentFactory {
    ...
    static getAgentsByType(engines) {
        return Array.from(this.agents.values()).filter(agent => engines.some(engine => agent.engines.includes(engine)));
    }
    static async executeAllAgents(topic, context) {
