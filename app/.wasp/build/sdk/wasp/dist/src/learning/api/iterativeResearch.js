...
function ensureTimestamp(timestamp) {
...
export function makeSerializable(result) {
    return {
        ...result,
        subtopicResults: Object.fromEntries(result.subtopicResults)
    };
}
export function makeIterativeFromSerializable(serialized) {
    return {
        ...serialized,
...
export class IterativeResearchEngine {
     ...
     */
    async researchAndGenerate(topic, options = {}, userContext) {
        ...
        try {
            ...
            if (mainResult.result.depth < maxDepth && mainResult.result.subtopics.length > 0) {
                ...
                this.processSubtopicsInBackground(mainResult.result.subtopics, topicId, maxDepth, options, userContext?.userId ? {
                    userId: userContext.userId,
                    level: userContext.level,
                    style: userContext.style
     ...
     */
    async researchSingleTopic(request, forceRefresh = false, userContext) {
     ...
     */
    async processSubtopicsInBackground(subtopics, mainTopicId, maxDepth, options, userContext) {
        try {
     ...
     */
    async storeToDatabase(result, topicSlug) {
     ...
     */
    async storeGeneratedContent(topicId, result) {
        try {
            ...
            try {
                ...
                if (topic) {
                    ...
                    // Store main content
                    await storeTopicContent(topicId, topic.slug, result.content.content, // MDX content
                    'generated', topic.depth || 0, {
                    ...
                    if (result.content.keyTakeaways.length > 0) {
                        await storeTopicContent(topicId, topic.slug, result.content.keyTakeaways.join('\n'), 'summary', topic.depth || 0, {
     ...
     */
    async getUserExistingContent(topic, userContext) {
        try {
            ...
            if (anyContent.length > 0) {
                ...
                if (adaptableContent) {
                    ...
                    const metadata = adaptableContent.metadata;
                    if (metadata.researchResult) {
     ...
     */
    async storeUserContent(topic, result, userContext) {
        try {
    ...
    // Helper methods
    generateCacheKey(topic, userContext) {
 ...
 */
export async function researchAndGenerate(topic, options = {}, userContext) {
 ...
 */
export async function researchAndStore(topic, topicSlug, options = {}, userContext) {
