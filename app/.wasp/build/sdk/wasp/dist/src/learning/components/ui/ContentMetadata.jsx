...
import { Clock, BookOpen, TrendingUp, Target, CheckCircle, Circle } from 'lucide-react';
...
export function determineComplexity(topic, content) {
...
export function extractLearningObjectives(topic) {
...
export function ContentMetadata({ topic, parentTopic, sources = [], estimatedReadTime, complexity, prerequisites = [], learningObjectives = [], topicPosition, className, compact = false }) {
    ...
    if (compact) {
        ...
        {/* Progress */}
        {topic.userProgress && (<div className="flex items-center gap-1">
            {topic.userProgress.completed ? (<CheckCircle className="w-3 h-3 text-green-600"/>) : (<Circle className="w-3 h-3"/>)}
          </div>)}
      </div>);
    }
    return (<div className={cn("space-y-4", className)}>
      {/* Topic Context and Relationship */}
      {parentTopic && (<div className="flex items-center gap-2 text-sm text-muted-foreground">
          <span>Part of:</span>
          <Badge variant="outline" className="text-xs">
            {parentTopic.title}
          </Badge>
          {topicPosition && (<span className="text-xs">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {/* Sources Count */}
        {sources.length > 0 && (<Badge variant="secondary" className="text-xs">
        ...
        {/* Progress Status */}
        {topic.userProgress && (<>
            {topic.userProgress.completed ? (<Badge variant="secondary" className="text-xs bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200">
                <CheckCircle className="w-3 h-3 mr-1"/>
                Completed
              </Badge>) : topic.userProgress.timeSpent > 0 ? (<Badge variant="outline" className="text-xs">
                <Clock className="w-3 h-3 mr-1"/>
      ...
      {/* Progress Bar */}
      {topic.userProgress && !topic.userProgress.completed && progressPercentage > 0 && (<div className="space-y-2">
      ...
      {/* Prerequisites */}
      {prerequisites.length > 0 && (<div className="space-y-2">
          <h4 className="text-sm font-medium text-muted-foreground">Prerequisites</h4>
          <div className="flex flex-wrap gap-1">
            {prerequisites.map((prereq, index) => (<Badge key={index} variant="outline" className="text-xs">
                {prereq}
              </Badge>))}
          </div>
        </div>)}
      ...
      {/* Learning Objectives */}
      {objectives.length > 0 && (<div className="space-y-2">
          <h4 className="text-sm font-medium text-muted-foreground">What you'll learn</h4>
          <ul className="space-y-1 text-sm text-muted-foreground">
            {objectives.slice(0, 3).map((objective, index) => (<li key={index} className="flex items-start gap-2">
      ...
      {sources.length > 0 && (<div className="space-y-2">
          ...
          <div className="space-y-1">
            {sources.slice(0, 3).map((source, index) => (<div key={source.id} className="flex items-center gap-2 text-xs text-muted-foreground">
                <div className="w-1.5 h-1.5 rounded-full bg-muted-foreground/50"/>
                {source.url ? (<a href={source.url} target="_blank" rel="noopener noreferrer" className="hover:text-foreground transition-colors underline">
                    {source.title}
                  </a>) : (<span>{source.title}</span>)}
                <Badge variant="outline" className="text-xs">
                  {source.source}
                </Badge>
                {source.relevanceScore && (<span className="text-xs opacity-75">
                    {Math.round(source.relevanceScore * 100)}% relevant
                  </span>)}
              </div>))}
            {sources.length > 3 && (<div className="text-xs text-muted-foreground">
      ...
      {topicPosition && (<div className="flex items-center gap-2 text-xs text-muted-foreground pt-2 border-t">
...
//# sourceMappingURL=ContentMetadata.jsx.map