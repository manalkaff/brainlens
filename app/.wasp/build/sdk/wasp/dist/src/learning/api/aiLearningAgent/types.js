...
export const ResearchPlanSchema = z.object({
    ...
    return (actualDistribution.general === data.engineDistribution.general &&
...
export const ContentStructureSchema = z.object({
    ...
    sections: z
        ...
        sources: z
            .array(z.string())
            .describe("Source references used in this section - must be strings only"),
        complexity: z
    ...
    keyTakeaways: z
        ...
        .min(3)
        .max(7)
        .describe("Main learning points that summarize the progressive understanding built through the content - must be an array of strings only, not objects"),
    nextSteps: z
        .array(z.string())
        .min(2)
        .max(5)
        .describe("Practical, actionable next learning steps that build on the knowledge gained - must be an array of strings only, not objects"),
}).refine((data) => {
    ...
    const hasFoundation = sectionTitles.slice(0, 2).some(title => title.includes('basic') ||
        title.includes('foundation') ||
        title.includes('introduction') ||
        title.includes('what is') ||
        title.includes('overview'));
    // Check for practical applications in later sections
    const hasApplication = sectionTitles.slice(-2).some(title => title.includes('application') ||
