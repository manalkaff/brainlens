...
export class AILearningAgent {
    // Module instances
    topicUnderstanding = new TopicUnderstandingModule();
    researchPlanning = new ResearchPlanningModule();
    researchExecution = new ResearchExecutionModule();
    synthesis = new SynthesisModule();
    contentGeneration = new ContentGenerationModule();
    subtopicIdentification = new SubtopicIdentificationModule();
    validation = new ValidationModule();
    fallback = new FallbackModule();
     ...
     */
    async researchAndGenerate(request) {
        ...
        try {
            ...
            const researchPlan = await this.researchPlanning.planResearch(request.topic, understanding, request.userContext);
            ...
            const synthesis = await this.synthesis.synthesizeResearch(request.topic, researchResults);
            ...
            const content = await this.generateContent(request.topic, synthesis);
            ...
            if (request.depth < request.maxDepth) {
                subtopics = await this.subtopicIdentification.identifySubtopics(request.topic, synthesis, request.depth + 1);
            }
            ...
            const sources = this.utils.buildSourceAttributions(researchResults, content.sections);
            ...
            const result = {
                ...
                metadata: {
                    ...
                    confidenceScore: this.utils.calculateConfidenceScore(researchResults, content),
                    lastUpdated: new Date(),
                },
                cacheKey: this.utils.generateCacheKey(request.topic, request.userContext),
                timestamp: new Date(),
            };
            console.log(`✅ Research completed in ${researchDuration}ms with ${sources.length} sources`);
            return result;
        }
        catch (error) {
     ...
     */
    async generateContent(topic, synthesis) {
        try {
            ...
            return this.fallback.createFallbackContent(topic, synthesis, error, this.utils.formatAsMDX.bind(this.utils), this.utils.estimateReadTime.bind(this.utils));
        }
    }
