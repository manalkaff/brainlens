...
export function MindMapTab() {
    ...
    const topics = useMemo(() => {
        if (!topic)
            return [];
        const flattenTopics = (topicItem) => {
            const result = [topicItem];
            if (topicItem.children) {
                topicItem.children.forEach((child) => {
    ...
    return (<div className="h-full w-full">
      <MindMapVisualization topics={topics} selectedTopicId={topic?.id} onTopicSelect={(selectedTopic) => {
            console.log('Selected topic:', selectedTopic);
        }}/>
    </div>);
}
//# sourceMappingURL=MindMapTab.jsx.map