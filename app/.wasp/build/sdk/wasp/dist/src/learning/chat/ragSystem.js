...
export class RAGSystem {
    ...
    constructor(maxTokens = 4000, temperature = 0.7) {
    ...
    async generateResponse(query, context) {
        ...
        try {
            ...
            const { expandedQuery, intent, queryTerms } = await this.preprocessQuery(query, context);
            // Step 2: Multi-stage retrieval with query expansion
            let ragContext;
            try {
                ragContext = await this.performEnhancedRetrieval(expandedQuery, context.topicId, {
                ...
                console.warn("Enhanced retrieval failed, falling back to basic retrieval:", retrievalError);
            ...
            const conversationContext = await this.buildEnhancedConversationContext(context.conversationHistory, Math.floor(this.defaultMaxTokens * 0.2), query);
            // Step 4: Create adaptive prompt template
            const promptTemplate = this.createAdaptivePromptTemplate(context, intent, ragContext.sources);
            // Step 5: Generate AI response with enhanced context
            const aiResponse = await this.generateEnhancedAIResponse(query, expandedQuery, ragContext.context, conversationContext, promptTemplate, context, intent);
            // Step 6: Generate intelligent follow-up questions
            const suggestedQuestions = await this.generateIntelligentFollowUpQuestions(query, aiResponse, context, ragContext.sources, intent);
            // Step 7: Calculate enhanced confidence score
            const confidence = this.calculateEnhancedConfidence(ragContext.sources, ragContext.totalTokens, aiResponse.length, intent, conversationContext.relevance);
            ...
            return await this.generateFallbackResponse(query, context, Date.now() - startTime);
        }
    }
    /**
     * Build enhanced conversation history context with intelligent summarization
     */
    async buildEnhancedConversationContext(history, maxTokens, currentQuery) {
        if (history.length === 0) {
            return { context: "", tokens: 0, relevance: 0 };
        }
        // Step 1: Calculate relevance scores for messages
        const relevantMessages = await this.scoreMessageRelevance(history, currentQuery);
        ...
        const averageRelevance = messageCount > 0 ? totalRelevance / messageCount : 0;
        return {
     ...
     */
    createAdaptivePromptTemplate(context, intent, sources) {
        ...
        const qualityInstructions = this.getQualitySpecificInstructions(sourceQuality);
...
5. Cites sources from the provided content`;
     ...
     */
    getLearningStyleInstructions(style) {
        switch (style) {
            case "visual":
     ...
     */
    async generateAIResponse(query, ragContext, conversationContext, promptTemplate, context) {
        const messages = [
            {
                role: "system",
     ...
     */
    async generateIntelligentFollowUpQuestions(originalQuery, response, context, sources, intent) {
        try {
...
3. [Question 3]`;
     ...
     */
    calculateEnhancedConfidence(sources, contextTokens, responseLength, intent, conversationRelevance = 0) {
        let confidence = 0;
        ...
        const intentBonus = intent === "specific" ? 0.1 : intent === "explanation" ? 0.08 : 0.05;
     ...
     */
    async searchRelevantContent(query, topicId, options = {}) {
...
Respond with just the category name:`;
     ...
     */
    async performEnhancedRetrieval(query, topicId, options) {
        // Primary retrieval
        ...
        if (primaryResults.sources.length < 3) {
            const expandedResults = await extractRAGContext(options.originalQuery, topicId, {
     ...
     */
    async generateEnhancedAIResponse(originalQuery, expandedQuery, ragContext, conversationContext, promptTemplate, context, intent) {
        const messages = [
            {
                role: "system",
                content: promptTemplate.systemPrompt,
        ...
        let userPrompt = promptTemplate.userPrompt.replace("{query}", originalQuery);
     ...
     */
    async generateFallbackResponse(query, context, processingTime) {
        return {
     ...
     */
    async scoreMessageRelevance(history, currentQuery) {
        return history.map((message, index) => {
            ...
            const overlap = queryWords.filter((word) => messageWords.includes(word)).length;
    ...
    assessSourceQuality(sources) {
        if (sources.length === 0)
            return "low";
        const avgScore = sources.reduce((sum, s) => sum + s.score, 0) / sources.length;
        const scoreVariance = sources.reduce((sum, s) => sum + Math.pow(s.score - avgScore, 2), 0) /
            sources.length;
        if (avgScore > 0.8)
            return "high";
        if (avgScore > 0.6)
            return scoreVariance < 0.1 ? "medium" : "mixed";
        return "low";
    }
    /**
     * Get quality-specific instructions
     */
    getQualitySpecificInstructions(quality) {
        switch (quality) {
            case "high":
     ...
     */
    async optimizeConversationHistory(history, maxMessages = 10) {
