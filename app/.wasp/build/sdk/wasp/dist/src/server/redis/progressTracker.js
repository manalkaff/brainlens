...
import { getProgressKey, getProgressLockKey, RESEARCH_STEPS, calculateOverallProgress, createInitialProgress } from '../../shared/progressTypes';
 ...
 */
export async function updateResearchProgress(topicId, update) {
    return await safeRedisOperationWithCircuitBreaker(async (client) => {
        ...
        try {
            ...
            if (update.status !== undefined)
                currentData.status = update.status;
            if (update.currentStep !== undefined)
                currentData.currentStep = update.currentStep;
            if (update.error !== undefined)
                currentData.error = update.error;
            ...
            currentData.totalProgress = calculateOverallProgress(update.progress || currentData.totalProgress, currentData.subtopicsProgress);
 ...
 */
export async function completeResearchStep(topicId, stepNumber, stepName, result, duration) {
    return await safeRedisOperationWithCircuitBreaker(async (client) => {
        const key = getProgressKey(topicId);
        const existingData = await client.hgetall(key);
        if (Object.keys(existingData).length === 0) {
        ...
        const completedSteps = JSON.parse(existingData.completedSteps || '[]');
        const stepDetails = JSON.parse(existingData.stepDetails || '{}');
 ...
 */
export async function completeMainTopic(topicId, result) {
    return await safeRedisOperationWithCircuitBreaker(async (client) => {
        const key = getProgressKey(topicId);
        const updateData = {
 ...
 */
export async function updateSubtopicProgress(topicId, subtopicProgress) {
    return await safeRedisOperationWithCircuitBreaker(async (client) => {
        const key = getProgressKey(topicId);
        const existingData = await client.hgetall(key);
        if (Object.keys(existingData).length === 0) {
 ...
 */
export async function getResearchProgress(topicId) {
    return await safeRedisOperationWithCircuitBreaker(async (client) => {
        const key = getProgressKey(topicId);
 ...
 */
export async function isResearchInProgress(topicId) {
 ...
 */
export async function getCompletedMainTopic(topicId) {
    const progress = await getResearchProgress(topicId);
    if (!progress || !progress.mainTopicCompleted)
        return null;
 ...
 */
export async function initializeProgress(topicId) {
 ...
 */
export async function setProgressExpiration(topicId, ttlSeconds) {
    return await safeRedisOperationWithCircuitBreaker(async (client) => {
        const key = getProgressKey(topicId);
 ...
 */
export async function clearResearchProgress(topicId) {
    return await safeRedisOperationWithCircuitBreaker(async (client) => {
        const key = getProgressKey(topicId);
        const lockKey = getProgressLockKey(topicId);
 ...
 */
export async function startResearchStep(topicId, stepNumber, message) {
 ...
 */
export async function setResearchError(topicId, error, currentStep) {
    const update = {
        topicId,
        status: 'error',
        currentStep,
 ...
 */
export async function completeAllResearch(topicId, mainTopicResult, message) {
    const update = {
        topicId,
        status: 'completed',
        progress: 100,
