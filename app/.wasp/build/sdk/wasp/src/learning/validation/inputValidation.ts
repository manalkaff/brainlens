...
// Research-specific validation functions

// Research complexity type
export type ResearchComplexity = 'simple' | 'moderate' | 'complex';

// Research input validation interface
...
export function validateResearchInput(input: string, context?: { userLevel?: string; topicDepth?: number }): ResearchInputValidation {
  const result: ResearchInputValidation = {
    isValid: true,
    sanitizedQuery: '',
    complexity: 'moderate',
    keywords: [],
...
export function calculateResearchComplexity(
  topic: string, 
  context?: { userLevel?: string; topicDepth?: number }
): ResearchComplexity {
  if (!topic) return 'simple';
  
  let complexityScore = 0;
  
  // Length-based complexity
  if (topic.length > 100) complexityScore += 1;
  if (topic.length > 200) complexityScore += 1;
  
  // Keyword complexity
  const keywords = extractTopicKeywords(topic);
  if (keywords.length > 5) complexityScore += 1;
  if (keywords.length > 10) complexityScore += 1;
  ...
  if (technicalTerms.test(topic)) complexityScore += 2;
  
  // Context-based adjustments
  if (context?.userLevel === 'beginner') complexityScore += 1;
  if (context?.userLevel === 'advanced') complexityScore -= 1;
  if (context?.topicDepth && context.topicDepth > 2) complexityScore += 1;
...
export function estimateResearchDuration(
  complexity: ResearchComplexity,
  keywordCount: number
): number {
  const baseTime = {
