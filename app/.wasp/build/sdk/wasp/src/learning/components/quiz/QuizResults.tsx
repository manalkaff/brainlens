...
interface Achievement {
  ...
  icon: string;
  earned: boolean;
  condition: (score: number, timeElapsed: number, quiz: Quiz) => boolean;
}
...
const achievements: Achievement[] = [
  ...
  },
  {
    ...
    condition: (score, timeElapsed, quiz) => {
      const quizWithQuestions = quiz as Quiz & { questions?: QuizQuestion[] };
      return (quizWithQuestions.questions?.length || 0) >= 10 && score >= 60;
    }
  }
];

export function QuizResults({ quiz, timeElapsed, onRetakeQuiz, onBackToQuizzes }: QuizResultsProps) {
  ...
  const earnedAchievements = achievements.filter(achievement => 
    achievement.condition(score, timeElapsed, quiz)
  );

  // Generate study recommendations
  const getStudyRecommendations = (): Array<{title: string; description: string; priority: string}> => {
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {earnedAchievements.map((achievement) => (
                <div key={achievement.id} className="text-center p-4 border rounded-lg bg-yellow-50 border-yellow-200">
