...
export class ServiceWorkerManager {
  ...
  private offlineStatusListeners: Array<(isOnline: boolean) => void> = [];
   ...
   */
  async cacheContent(url: string, content: string): Promise<boolean> {
   ...
   */
  async enableBackgroundSync(tag: string = 'offline-actions'): Promise<boolean> {
    if (!this.registration) return false;
  ...
  async enablePushNotifications(): Promise<boolean> {
    if (!('Notification' in window) || !this.registration) return false;
  ...
  private notifyOfflineStatusListeners(isOnline: boolean): void {
