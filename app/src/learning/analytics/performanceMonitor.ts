...
interface ResourceMetrics {
  ...
  fastestResource: { name: string; duration: number };
}
...
export class PerformanceMonitor {
  ...
  private readonly STORAGE_KEY = 'brainlens_performance_metrics';
  ...
  getNavigationMetrics(): NavigationMetrics | null {
    if (!window.performance?.navigation) return null;
  ...
  private recordNavigationMetrics(): void {
    if (!window.performance?.navigation) return;
  ...
  private updateCustomMetrics(name: string, value: number): void {
