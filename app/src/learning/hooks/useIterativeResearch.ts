...
import { 
  ...
  generateSubtopics,
  getTopicHierarchy,
  checkResearchFreshness
} from '../operations/iterativeResearchClient';
import type { IterativeResearchResult } from '../api/iterativeResearch';
import type { TopicHierarchy, DepthExpansionResult } from '../api/topicDepthManager';
...
export interface UseIterativeResearchReturn {
  ...
  // Actions
  startResearch: (options?: { forceRefresh?: boolean }) => Promise<void>;
 ...
 */
export function useIterativeResearch(options: UseIterativeResearchOptions = {}): UseIterativeResearchReturn {
  const {
    topicSlug,
    autoStart = false,
    maxDepth = 3,
  ...
  const generateTopicSubtopics = useCallback(async (topicId: string) => {
    if (isResearching) return;
  ...
  const isTopicExpanded = useCallback((topicId: string): boolean => {
    if (!hierarchy) return false;
    
    const findTopic = (node: TopicHierarchy): boolean => {
      if (node.id === topicId) {
  ...
  const getResearchStats = useCallback(() => {
    ...
    if (hierarchy) {
      const flattenHierarchy = (node: TopicHierarchy): TopicHierarchy[] => {
 ...
 */
export function useResearchFreshness(topicId: string | null, cacheTtlDays: number = 7) {
