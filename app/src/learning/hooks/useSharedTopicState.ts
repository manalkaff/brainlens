...
// Shared state that persists across tabs
...
const defaultState: SharedTopicState = {
...
export function useSharedTopicState() {
  ...
  const [sharedState, setSharedState] = useState<SharedTopicState>(defaultState);
  ...
  // Update methods
  const updateLearningPreferences = useCallback((preferences: Partial<Pick<SharedTopicState, 'learningStyle' | 'knowledgeLevel'>>) => {
  ...
  const updateUIPreferences = useCallback((preferences: Partial<Pick<SharedTopicState, 'sidebarCollapsed' | 'preferredLayout'>>) => {
    setSharedState(prev => ({ ...prev, ...preferences }));
  }, []);
  ...
  return {
    ...
    removeBookmark,
    toggleBookmark,
    isBookmarked: (sectionId: string) => sharedState.bookmarkedSections.includes(sectionId),
