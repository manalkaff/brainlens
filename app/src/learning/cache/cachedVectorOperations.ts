 ...
 */
export async function searchTopicContentCached(
  query: string,
  topicId: string,
  options: {
    limit?: number;
 ...
 */
export async function searchAllContentCached(
  query: string,
  options: {
    limit?: number;
    scoreThreshold?: number;
 ...
 */
export async function getContentRecommendationsCached(
  topicId: string,
  topicTitle: string,
  options: {
    limit?: number;
 ...
 */
export async function extractRAGContextCached(
  query: string,
  topicId: string,
  options: {
    maxTokens?: number;
 ...
 */
export async function storeTopicContentCached(
  topicId: string,
  topicSlug: string,
  content: string,
 ...
 */
export async function storeTopicContentBatchCached(
  topicId: string,
  topicSlug: string,
  contentItems: Array<{
    content: string;
 ...
 */
export async function deleteTopicVectorContentCached(topicId: string): Promise<void> {
  try {
 ...
 */
export async function invalidateTopicSearchCache(topicId: string): Promise<void> {
  await cacheService.invalidateSearchResults(topicId);
}
...
export async function warmSearchCache(
  ...
  for (const topicId of topicIds) {
    for (const query of commonQueries) {
      ...
      promises.push(
        searchTopicContentCached(query, topicId, { limit: 10 })
