...
export function PerformanceDashboard({ className = '' }: PerformanceDashboardProps) {
  ...
  const [selectedView, setSelectedView] = useState<'overview' | 'navigation' | 'resources' | 'memory' | 'custom'>('overview');
  ...
  const getStatusIcon = (status: MetricCard['status']) => {
    switch (status) {
  ...
  const renderOverview = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {metricCards.slice(0, 8).map((metric, index) => (
  ...
  const renderDetailedView = () => {
    switch (selectedView) {
      case 'navigation':
        ...
        ) : <div>No navigation data available</div>;
        ...
        return (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {summary.custom.topicLoadTime.length > 0 && (
                <Card>
                  <CardContent className="p-4">
              ...
              {summary.custom.chatResponseTime.length > 0 && (
                <Card>
                  <CardContent className="p-4">
              ...
              {summary.custom.mindMapRenderTime.length > 0 && (
                <Card>
                  <CardContent className="p-4">
