...
export function ProgressOverlay({ 
  ...
  const getAchievementLevel = (completionRate: number) => {
    if (completionRate >= 90) return { level: 'Master', color: 'text-purple-600', icon: Award };
    if (completionRate >= 75) return { level: 'Expert', color: 'text-blue-600', icon: Target };
    if (completionRate >= 50) return { level: 'Advanced', color: 'text-green-600', icon: TrendingUp };
    if (completionRate >= 25) return { level: 'Learning', color: 'text-yellow-600', icon: Zap };
  ...
  return (
    <Card className={`${className} bg-gradient-to-br from-white/95 to-gray-50/95 backdrop-blur-sm border shadow-lg`}>
      <CardContent className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2 text-xs">
          <Button
            variant="outline"
            size="sm"
            onClick={() => onFocusProgress('completed')}
            className="h-auto p-2 flex flex-col items-center gap-1 hover:bg-green-50 border-green-200"
          >
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => onFocusProgress('inProgress')}
            className="h-auto p-2 flex flex-col items-center gap-1 hover:bg-blue-50 border-blue-200"
          >
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => onFocusProgress('notStarted')}
            className="h-auto p-2 flex flex-col items-center gap-1 hover:bg-gray-50 border-gray-200"
          >
        ...
        {/* Additional Metrics */}
        {(stats.totalTimeSpent || stats.averageSessionTime) && (
...
export function CompactProgressOverlay({ 
  ...
  return (
    <div className={`${className} bg-white/90 backdrop-blur-sm border rounded-lg p-3 shadow-sm`}>
