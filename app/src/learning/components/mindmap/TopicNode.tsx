...
export const TopicNode = memo(({ data, selected }: TopicNodeProps) => {
  ...
  const getNodeColor = () => {
    if (isCompleted) return 'bg-emerald-50 border-emerald-300 text-emerald-900';
    if (hasProgress) return 'bg-blue-50 border-blue-300 text-blue-900';
    return 'bg-white border-gray-200 text-gray-900';
  };

  // Get status icon
  const getStatusIcon = () => {
    if (isCompleted) return <CheckCircle className="w-4 h-4 text-emerald-600" />;
