...
export const RESEARCH_STEPS: Array<{
  stepNumber: number;
  name: string;
  description: string;
...
export class ProgressTracker {
  private readonly PROGRESS_TTL_SECONDS = 24 * 60 * 60; // 24 hours
  private readonly KEY_PREFIX = 'research:progress';
  
  private getProgressKey(topicId: string): string {
    return `${this.KEY_PREFIX}:${topicId}`;
  }

  async initializeResearch(topicId: string, options: {
    topicTitle?: string;
    status?: string;
    message?: string;
  }): Promise<void> {
  ...
  async startStep(topicId: string, stepNumber: number, customMessage?: string): Promise<void> {
  ...
  async completeStep(topicId: string, stepNumber: number, stepName: string, result: any, duration: number): Promise<void> {
    const progress = await this.getProgress(topicId);
    if (!progress) {
      console.error(`Progress not found for topic: ${topicId}`);
      return;
  ...
  async completeMainTopic(topicId: string, result: any): Promise<void> {
    const progress = await this.getProgress(topicId);
    if (!progress) {
      console.error(`Progress not found for topic: ${topicId}`);
      return;
  ...
  async updatePhase(topicId: string, phase: ResearchProgress['phase'], message: string): Promise<void> {
    const progress = await this.getProgress(topicId);
    if (!progress) {
      console.error(`Progress not found for topic: ${topicId}`);
      return;
  ...
  async updateSubtopicProgress(topicId: string, subtopicTitle: string, subtopicProgress: Partial<SubtopicProgress>): Promise<void> {
    const progress = await this.getProgress(topicId);
    if (!progress) {
      console.error(`Progress not found for topic: ${topicId}`);
      return;
  ...
  async completeResearch(topicId: string): Promise<void> {
    const progress = await this.getProgress(topicId);
    if (!progress) {
      console.error(`Progress not found for topic: ${topicId}`);
      return;
  ...
  async setError(topicId: string, error: string): Promise<void> {
    const progress = await this.getProgress(topicId);
    if (!progress) {
      console.error(`Progress not found for topic: ${topicId}`);
      return;
  ...
  async getProgress(topicId: string): Promise<ResearchProgress | null> {
    const key = this.getProgressKey(topicId);
    return await redisCache.get<ResearchProgress>(key);
  }

  async setCompleted(topicId: string, message: string): Promise<void> {
    const progress = await this.getProgress(topicId);
    if (!progress) {
  ...
  async getProgressWithFallback(topicId: string): Promise<ResearchProgress | null> {
