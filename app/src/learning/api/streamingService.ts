...
import { aiContentGenerator, type ContentGenerationOptions, type ResearchResult } from './contentGenerator';
...
class StreamConnectionManager {
  ...
  createStream(streamId: string, res: Response): void {
    ...
    this.sendEvent(res, 'connected', { streamId, timestamp: new Date().toISOString() });
  ...
  closeStream(streamId: string): void {
    const res = this.connections.get(streamId);
    if (res) {
      this.sendEvent(res, 'close', { streamId, timestamp: new Date().toISOString() });
      res.end();
      this.connections.delete(streamId);
      this.streamStatus.delete(streamId);
    }
      ...
      for await (const chunk of result.textStream) {
        ...
        this.connectionManager.sendToStream(streamId, 'content', {
          ...
          accumulatedContent: accumulatedContent.length > 1000 ? 
            accumulatedContent.slice(-1000) + '...' : accumulatedContent,
...
export async function createStreamHandler(req: Request, res: Response, context: any) {
  try {
...
export async function streamContentHandler(req: Request, res: Response, context: any) {
  try {
    ...
    streamingContentService.streamLearningContent(
      streamId,
      topic,
      researchResults || [],
      options || {}
